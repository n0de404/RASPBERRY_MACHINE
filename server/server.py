...
SERVER_APP_LOG_MAX_BYTES = max(
    1_000_000,
    int(os.environ.get("MACHINE_SERVER_APP_LOG_MAX_BYTES", "20000000") or 20_000_000),
)
DASHBOARD_INITIAL_HISTORY_LIMIT = max(
    0,
    int(os.environ.get("MACHINE_DASHBOARD_INITIAL_HISTORY_LIMIT", "10") or 10),
)
...
ADDITIONAL_MACHINE_CODES = {
    "M00111", "M00112", "M00113", "M00114", "M00115", "M00116", "M00117", "M00118",
...
# Temporary server-side containment for clients replaying old finish history.
# Set MACHINE_SERVER_ACCEPT_FINISH_EVENTS=1 when finish-event ingestion should
# be enabled again. Heartbeats and live session snapshots remain unaffected.
SERVER_ACCEPT_FINISH_EVENTS = str(os.environ.get("MACHINE_SERVER_ACCEPT_FINISH_EVENTS", "0")).strip().lower() in {
    "1", "true", "yes", "on",
}
...
CLIENT_IDENTITY_LEASE_SECONDS = max(
    10,
    int(os.environ.get("MACHINE_CLIENT_IDENTITY_LEASE_SECONDS", "20")),
)
...
def _append_server_app_log(kind: str, row: Dict[str, Any]) -> None:
    try:
        ...
        # Successful heartbeats are high-frequency health signals, not useful
        # audit records. Avoid two synchronous disk writes every five seconds
        # for every connected machine.
        if (
            kind_text in {"api_event_received", "api_event_ok"}
            and str(row_data.get("event_type") or "").strip().upper() == "HEARTBEAT"
        ):
            return
        item = {"at_utc": utc_now().isoformat(), "kind": kind_text, **row_data}
        SERVER_APP_LOG_FILE.parent.mkdir(parents=True, exist_ok=True)
        if SERVER_APP_LOG_FILE.exists() and SERVER_APP_LOG_FILE.stat().st_size >= SERVER_APP_LOG_MAX_BYTES:
            rotated = SERVER_APP_LOG_FILE.with_suffix(SERVER_APP_LOG_FILE.suffix + ".1")
            if rotated.exists():
                rotated.unlink()
            SERVER_APP_LOG_FILE.replace(rotated)
...
def _load_processed_event_ids() -> List[str]:
    try:
        raw = json.loads(PROCESSED_EVENT_IDS_FILE.read_text(encoding="utf-8-sig"))
        rows = raw if isinstance(raw, list) else []
    except Exception:
        rows = []
    return [str(x).strip() for x in rows if isinstance(x, str) and str(x).strip()][-20000:]


PROCESSED_EVENT_IDS: List[str] = _load_processed_event_ids()
PROCESSED_EVENT_ID_SET = set(PROCESSED_EVENT_IDS)


def _mark_event_id_processed(event_id: str) -> None:
    event_key = str(event_id or "").strip()
    if not event_key or event_key in PROCESSED_EVENT_ID_SET:
        return
    PROCESSED_EVENT_IDS.append(event_key)
    PROCESSED_EVENT_ID_SET.add(event_key)
    if len(PROCESSED_EVENT_IDS) > 20000:
        removed = PROCESSED_EVENT_IDS[:-20000]
        del PROCESSED_EVENT_IDS[:-20000]
        for old_key in removed:
            PROCESSED_EVENT_ID_SET.discard(old_key)
    # MySQL is authoritative. Keep the legacy JSON file only as an emergency
    # fallback when SQL is unavailable during a transition or outage.
    if not _insert_processed_event_id_sql(event_key):
        _save_json_list(PROCESSED_EVENT_IDS_FILE, PROCESSED_EVENT_IDS)
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS `processed_event_ids` (
                  ...
                  `processed_at_utc` VARCHAR(50) NOT NULL,
                  PRIMARY KEY (`event_id`),
                  KEY `idx_processed_event_ids_time` (`processed_at_utc`)
                ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
                """
            )
            cur.execute(
            ...
            cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS `archived_jobs_server` (
                  ...
                  `part_availability_carryover_logs` JSON NULL,
                  `product_part_excess_logs` JSON NULL,
            ...
            try:
                ...
                pass
            legacy_required_json_columns = {
                "reject_breakdown",
                "job_payload",
                "raw_material_scans",
                "raw_material_logs",
                "raw_material_unique_keys",
                "product_pack_history_logs",
                "reject_review_logs",
                "linkage_job_payload",
                "linkage_jobs",
                "operator_shift_logs",
                "operator_shift_baseline_reject_breakdown",
            }
            try:
                cur.execute(
                    "SELECT `COLUMN_NAME`,`IS_NULLABLE` FROM `INFORMATION_SCHEMA`.`COLUMNS` "
                    "WHERE `TABLE_SCHEMA`=DATABASE() AND `TABLE_NAME`='active_machine_sessions'"
                )
                column_nullability = {
                    str(row.get("COLUMN_NAME") or ""): str(row.get("IS_NULLABLE") or "").upper()
                    for row in cur.fetchall() or []
                }
                for column_name in sorted(legacy_required_json_columns):
                    if column_nullability.get(column_name) == "NO":
                        cur.execute(
                            f"ALTER TABLE `active_machine_sessions` "
            ...
            for stmt in (
                ...
                "ALTER TABLE `finished_jobs` ADD COLUMN `part_availability_carryover_logs` JSON NULL AFTER `raw_material_logs`",
                "ALTER TABLE `finished_jobs` ADD COLUMN `product_part_excess_logs` JSON NULL AFTER `part_availability_carryover_logs`",
                "ALTER TABLE `archived_jobs_server` ADD COLUMN `part_availability_carryover_logs` JSON NULL AFTER `raw_material_logs`",
                "ALTER TABLE `archived_jobs_server` ADD COLUMN `product_part_excess_logs` JSON NULL AFTER `part_availability_carryover_logs`",
            ):
                try:
                    cur.execute(stmt)
                except Exception:
            ...
            for row in rows:
                ...
                cur.execute(
                    ...
                    (`finished_at_utc`,`client_id`,`machine_code`,`machine_name`,`job_code`,`job_name`,`operator_id`,
                     ...
                     `reject_breakdown`,`raw_material_scans`,`raw_material_logs`,`part_availability_carryover_logs`,`product_part_excess_logs`,`job_payload`,`reject_review_logs`,
                    ...
                    (%s,%s,%s,%s,%s,%s,%s,
                     ...
                     %s,%s,%s,%s,%s,%s,
                     %s,%s,%s,%s,%s,
                     CAST(%s AS JSON),CAST(%s AS JSON),CAST(%s AS JSON),CAST(%s AS JSON),CAST(%s AS JSON),CAST(%s AS JSON),CAST(%s AS JSON),
                    ...
                    (
                        ...
                        json.dumps(row.get("reject_breakdown", {}), ensure_ascii=False), json.dumps(row.get("raw_material_scans", []), ensure_ascii=False), json.dumps(row.get("raw_material_logs", []), ensure_ascii=False), json.dumps(row.get("part_availability_carryover_logs", []), ensure_ascii=False), json.dumps(row.get("product_part_excess_logs", []), ensure_ascii=False), json.dumps(row.get("job_payload", {}), ensure_ascii=False), json.dumps(row.get("reject_review_logs", []), ensure_ascii=False),
...
def _load_active_sessions_sql() -> Optional[Dict[str, Dict[str, Any]]]:
    conn = _sql_conn()
    if conn is None:
        return None
    try:
        out: Dict[str, Dict[str, Any]] = {}
        with conn.cursor() as cur:
            cur.execute("SELECT `machine_code`, `raw_json` FROM `active_machine_sessions`")
            for db_row in cur.fetchall() or []:
                item = _sql_decode_json((db_row or {}).get("raw_json"), {})
                code = str((db_row or {}).get("machine_code") or item.get("machine_code") or "").strip()
                if code and isinstance(item, dict):
                    item["machine_code"] = code
                    out[code] = item
        return out
    except Exception:
        return None
...
def _load_processed_event_ids_sql() -> Optional[List[str]]:
    conn = _sql_conn()
    if conn is None:
        return None
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT `event_id` FROM `processed_event_ids` "
                "ORDER BY `processed_at_utc` DESC LIMIT 20000"
            )
            rows = [str((row or {}).get("event_id") or "").strip() for row in cur.fetchall() or []]
        return [value for value in reversed(rows) if value]
    except Exception:
        return None
    finally:
        conn.close()
...
def _insert_processed_event_id_sql(event_id: str) -> bool:
    event_key = str(event_id or "").strip()
    if not event_key:
        return False
    conn = _sql_conn()
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT IGNORE INTO `processed_event_ids` (`event_id`,`processed_at_utc`) VALUES (%s,%s)",
...
def _save_processed_event_ids_sql(event_ids: List[str]) -> bool:
    migrated_at = datetime.now(timezone.utc).isoformat()
    values = [(str(value or "").strip(), migrated_at) for value in event_ids]
    values = [value for value in values if value[0]][-20000:]
    conn = _sql_conn()
    if conn is None:
        return False
    try:
...
def _load_server_document_sql(document_key: str) -> Optional[Dict[str, Any]]:
    conn = _sql_conn()
    if conn is None:
        return None
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT `raw_json` FROM `server_documents` WHERE `document_key`=%s LIMIT 1",
                (str(document_key or "").strip(),),
            )
            row = cur.fetchone()
        if not row:
            return {}
        value = _sql_decode_json(row.get("raw_json"), {})
...
def _save_server_document_sql(document_key: str, payload: Dict[str, Any]) -> bool:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO `server_documents` (`document_key`,`updated_at_utc`,`raw_json`)
...
def _normalize_machine_code_from_scan(value: Any) -> str:
    ...
    if 301 <= machine_no <= 321 or machine_no in {
...
class MachineSession:
    ...
    pdr_downtime_logs: List[Dict[str, Any]] = None
    ...
    machine_counter_override_value: Optional[int] = None
    machine_counter_override_app_delta_baseline: Optional[int] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        # versions synthesized PACK_EVENT_FALLBACK rows from aggregate counters;
        # those rows have no series/lot/PO and were also persisted as real data.
        d["product_pack_history_logs"] = [
            row
            for row in (d["product_pack_history_logs"] or [])
        ...
        d["downtime_total_seconds"] = sum(
            max(0, int(float(row.get("duration_seconds") or 0)))
            for row in d["pdr_downtime_logs"]
            if isinstance(row, dict)
        )
        if (
            d.get("downtime_active")
            and d.get("downtime_started_at")
        ):
            d["downtime_total_seconds"] += max(0, int(d.get("downtime_last_seconds") or 0))
            d["downtime_total_seconds"] += max(0, int(time.time() - float(d["downtime_started_at"])))
...
def _reset_active_session_for_new_job_segment(sess: MachineSession, preserve_operator_shift_logs: bool = False) -> None:
    ...
    sess.machine_counter_override_value = None
    sess.machine_counter_override_app_delta_baseline = None
    ...
    sess.linkage_job_code = None
    sess.linkage_job_name = None
    sess.linkage_job_payload = {}
    sess.linkage_jobs = []
...
def _job_payload_identity_keys(payload: Any) -> set[str]:
    src = payload if isinstance(payload, dict) else {}
    data_obj = src.get("data") if isinstance(src.get("data"), dict) else {}
    job = data_obj.get("job") if isinstance(data_obj.get("job"), dict) else (
        src.get("job") if isinstance(src.get("job"), dict) else {}
    )
    return {
...
def _clear_stale_job_payload_and_pdr_wait(sess: MachineSession) -> None:
    """Remove job-scoped state that belongs to a different live job.

    Production totals and completed downtime history are preserved. Only the
    mismatched payload and active/waiting PDR workflow are cleared.
    """
    sess.job_payload = {}
    sess.downtime_reason_code = None
    sess.downtime_reason_text = None
...
def _clear_resolved_pdr_workflow(sess: MachineSession) -> None:
    """Clear only live PDR workflow state while preserving completed history."""
    sess.downtime_reason_code = None
    sess.downtime_reason_text = None
    sess.downtime_started_at = None
    sess.downtime_active = False
...
def _clear_completed_stale_pdr_wait(sess: MachineSession) -> bool:
    """Repair a persisted wait flag already superseded by a completed PDR."""
    if sess.downtime_active:
        return False
    waiting = bool(
        sess.waiting_downtime_start_maintenance
        or sess.waiting_pdr_maintenance_reason
        or sess.waiting_downtime_end_maintenance
        or sess.waiting_maintenance_qr
        or sess.waiting_supervisor_qr
        or sess.downtime_wait_started_at
    )
    if not waiting:
        return False
    wait_started = float(sess.downtime_wait_started_at or 0)
    latest_finished = 0.0
    for row in (sess.pdr_downtime_logs or []):
        if not isinstance(row, dict):
            continue
        raw_finished = str(
        ...
        if not raw_finished:
            continue
        try:
            finished_dt = datetime.fromisoformat(raw_finished.replace("Z", "+00:00"))
            if finished_dt.tzinfo is None:
                finished_dt = finished_dt.replace(tzinfo=timezone.utc)
            latest_finished = max(latest_finished, finished_dt.timestamp())
        except Exception:
            continue
    if latest_finished <= 0 or (wait_started > 0 and latest_finished < wait_started):
        return False
    _clear_resolved_pdr_workflow(sess)
    return True
...
def _session_from_active_snapshot(raw: Dict[str, Any]) -> Optional[MachineSession]:
    ...
    return MachineSession(
        ...
        product_pack_history_logs=[
            row
            for row in (raw.get("product_pack_history_logs") or [])
            if isinstance(row, dict)
            and str(row.get("source") or "").strip().upper() != "PACK_EVENT_FALLBACK"
        ],
        ...
        pdr_downtime_logs=[
            dict(row) for row in (raw.get("pdr_downtime_logs") or [])
            if isinstance(row, dict)
        ],
        ...
        cycle_time_change_logs=[
            dict(row) for row in (raw.get("cycle_time_change_logs") or [])
            if isinstance(row, dict)
        ],
        ...
        machine_counter_override_value=_parse_int_or_none(raw.get("machine_counter_override_value")),
        machine_counter_override_app_delta_baseline=_parse_int_or_none(
            raw.get("machine_counter_override_app_delta_baseline")
        ),
        machine_counter_overwrite_logs=[
            dict(row) for row in (raw.get("machine_counter_overwrite_logs") or [])
            if isinstance(row, dict)
        ],
        ...
        linkage_job_code=raw.get("linkage_job_code"),
        linkage_job_name=raw.get("linkage_job_name"),
        linkage_job_payload=dict(raw.get("linkage_job_payload") or {}),
...
def _update_client_status(
    ...
    *,
    ...
    row.pop("identity_conflict", None)
    row.pop("conflict_remote_host", None)
    row.pop("conflict_last_seen_utc", None)
    code = str(machine_code or "").strip()
    row.update({
...
def _active_client_identity_conflict(client_id: str, remote_host: str) -> Optional[Dict[str, Any]]:
    """Return the current owner when another live host is using this client ID."""
    cid = str(client_id or "").strip()
    host = str(remote_host or "").strip()
    if not cid or not host:
        return None
    row = CLIENT_STATUSES.get(_client_status_key(cid, host))
    if not isinstance(row, dict):
        return None
    owner_host = str(row.get("remote_host") or "").strip()
    if not owner_host or owner_host == host:
        return None
    try:
        last_seen = datetime.fromisoformat(str(row.get("last_seen_utc") or "").replace("Z", "+00:00"))
        if last_seen.tzinfo is None:
            last_seen = last_seen.replace(tzinfo=timezone.utc)
        age = (utc_now() - last_seen.astimezone(timezone.utc)).total_seconds()
    except Exception:
        return None
    return row if age <= float(CLIENT_IDENTITY_LEASE_SECONDS) else None


def _client_status_rows() -> List[Dict[str, Any]]:
    rows_by_client_machine: Dict[str, Dict[str, Any]] = {}

    def row_key(client_id: Any, remote_host: Any, machine_code: Any) -> str:
        cid = str(client_id or "").strip()
        host = str(remote_host or "").strip()
        code = str(machine_code or "").strip()
        identity = cid or host or "UNKNOWN"
        # A duplicated client ID is a configuration problem, but it must not
        # hide otherwise valid machine sessions from the admin list.
        return f"{identity.casefold()}|{code.casefold()}" if code else identity.casefold()

    for item in CLIENT_STATUSES.values():
        if not isinstance(item, dict):
            continue
        row = dict(item)
        key = row_key(row.get("client_id", ""), row.get("remote_host", ""), row.get("machine_code", ""))
        rows_by_client_machine[key] = row

    # Active sessions are persisted across server restarts, while the lightweight
    # client-status cache is not. Include them so Settings always shows every
    # active client, even before its next heartbeat reaches the restarted server.
    for sess in SESSIONS.values():
        client_id = str(sess.client_id or "").strip()
        if not client_id:
            continue
        key = row_key(client_id, "", sess.machine_code)
        row = dict(rows_by_client_machine.get(key) or {})
        row.setdefault("client_id", client_id)
        row.setdefault("remote_host", "")
        row["machine_code"] = str(sess.machine_code or "").strip()
        row["machine_name"] = _machine_display_name(sess.machine_code, sess.machine_name)
        row["machine_scanned"] = bool(sess.machine_code)
        row["active_session"] = True
        row["job_code"] = str(sess.job_code or "").strip()
        row["job_name"] = str(sess.job_name or "").strip()
        session_seen = str(sess.last_seen_utc or "").strip()
        if session_seen > str(row.get("last_seen_utc") or ""):
            row["last_seen_utc"] = session_seen
            row["last_event"] = str(sess.last_event or "").strip()
        rows_by_client_machine[key] = row

    rows = list(rows_by_client_machine.values())
...
def _build_job_queue_rows() -> List[Dict[str, Any]]:
    ...
    for sess in SESSIONS.values():
        ...
        if last_seen_dt is not None:
            try:
                if (now_utc - last_seen_dt).total_seconds() > JOB_QUEUE_STALE_SECONDS:
                    continue
            except Exception:
                pass
        ...
        if not is_connected:
            ...
            status = "NO TARGET"
        elif remaining_qty > 0 and act_cycle_seconds is None and live_cycle_seconds is None:
            status = "NO CYCLE"
        ...
        rows.append(
            {
                ...
                "target_reached": bool(target_qty > 0 and produced_now >= target_qty),
...
def _build_finished_job_qr_plan(finished_job: Dict[str, Any], product_id: str, po_number: str) -> List[Dict[str, Any]]:
    ...
    part_excess_rows = [
        ...
        if isinstance(item, dict)
    ] if isinstance(row.get("product_part_excess_logs"), list) else []
    for excess_index, excess_row in enumerate(part_excess_rows, start=1):
        available_qty = max(
            0.0,
        ...
        excess_product_id = str(
            excess_row.get("material_product_id")
            or excess_row.get("product_id")
            or ""
        ).strip()
        excess_sku = str(excess_row.get("material_sku") or excess_row.get("job_part_sku") or "").strip()
        excess_name = str(
            excess_row.get("material_name")
            or excess_row.get("material")
            or excess_row.get("job_part_name")
            or "Product Part Excess"
        ).strip()
        excess_meta = _lookup_product_meta(excess_product_id) if excess_product_id else _lookup_product_meta_by_text(excess_sku or excess_name)
        plan.append(
            {
                "stage_kind": "PRODUCT_PART_EXCESS",
                "stage_title": f"Product Part Excess - {excess_sku or excess_name}",
...
def load_server_settings() -> Dict[str, Any]:
    ...
    if isinstance(raw_visible, list):
        ...
        # Newly configured additional machines should become visible without
        # requiring an administrator to reset an older saved visibility list.
        for code in MACHINE_NAME_MAP:
            if code in ADDITIONAL_MACHINE_CODES and code not in seen_codes:
                seen_codes.add(code)
                visible_machine_codes.append(code)
    else:
    ...
    for it in rows:
        ...
        default_qty = next(
            (
                it.get(key)
                for key in (
                    "default_qty", "defaultQty", "default_quantity", "defaultQuantity",
                    "pack_qty", "packQty", "packing_qty", "packingQty",
                    "qty_per_pack", "qtyPerPack", "standard_qty", "standardQty",
                    "qty", "quantity",
                )
        ...
        if default_qty in (None, ""):
            # Some BMS raw-material products store their sack size only in the
            # product name (for example, "PP Resin (25 KGS)"). Normalize that
            # server-owned package size for QR consumers.
            package_match = re.search(
                r"(?<!\d)(\d+(?:\.\d+)?)\s*(?:kg|kgs|kilogram|kilograms)\b",
                name,
                flags=re.IGNORECASE,
            )
            if package_match:
                default_qty = package_match.group(1)
        if pid and name:
            out.append({
                "id": pid,
                "name": name,
...
def load_planning_board() -> Dict[str, Any]:
    raw = _load_server_document_sql("planning_board")
    if raw is None:
        raw = _load_json_object(PLANNING_BOARD_FILE)
    elif not raw:
        legacy = _load_json_object(PLANNING_BOARD_FILE)
        if legacy:
            raw = legacy
            _save_server_document_sql("planning_board", legacy)
...
def save_planning_board(board: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not _save_server_document_sql("planning_board", payload):
...
def _enrich_finished_job_identity(row: Dict[str, Any]) -> Dict[str, Any]:
    """Fill a code-only finished-shift identity from BMS when available."""
    item = dict(row or {})
    job_code = str(item.get("job_code") or "").strip()
    job_name = str(item.get("job_name") or "").strip()
    if not job_code or (job_name and job_name.upper() != job_code.upper()):
        return item
    try:
        payload = fetch_job_payload_from_bms(job_code)
    except Exception as exc:
        _append_server_app_log(
            "finished_shift_job_identity_lookup_failed",
            {"job_code": job_code, "error": str(exc)},
        )
        return item
    data = payload.get("data") if isinstance(payload.get("data"), dict) else {}
    job = data.get("job") if isinstance(data.get("job"), dict) else {}
    resolved_name = str(job.get("ref_no") or job.get("reference_no") or "").strip()
    if not resolved_name:
        return item
    item["job_name"] = resolved_name
    item["job_payload"] = payload
    if job.get("product_id") not in (None, ""):
        item["product_id"] = str(job.get("product_id"))
    if str(item.get("linkage_role") or "").strip().upper() == "LINKED":
        item["linkage_job_name"] = resolved_name
        item["linkage_job_payload"] = payload
        linked_rows = item.get("linkage_jobs") if isinstance(item.get("linkage_jobs"), list) else []
        item["linkage_jobs"] = [
            {
                **linked,
                ...
                and str(linked.get("job_code") or "").strip() == job_code
            ) else linked
...
def get_products(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    if fetched:
        old_set = {
            (str(x.get("id", "")), str(x.get("name", "")), str(x.get("default_qty", "")))
            for x in cached_items if isinstance(x, dict)
        }
        new_set = {
            (str(x.get("id", "")), str(x.get("name", "")), str(x.get("default_qty", "")))
            for x in fetched
...
def _migrate_legacy_server_json_to_sql() -> None:
    """Seed missing SQL state from legacy JSON without overwriting newer SQL rows."""
    global SESSIONS, PROCESSED_EVENT_IDS, PROCESSED_EVENT_ID_SET

    sql_active = _load_active_sessions_sql()
    try:
        legacy_active = _read_active_sessions_payload(ACTIVE_MACHINE_SESSIONS_FILE)
    except Exception:
        legacy_active = {}
    if isinstance(sql_active, dict) and isinstance(legacy_active, dict):
        for machine_code, row in legacy_active.items():
            code = str(machine_code or "").strip()
            if not code or code in sql_active or not isinstance(row, dict):
                continue
            seed = dict(row)
            seed["machine_code"] = code
            if _upsert_active_session_sql(seed):
                sql_active[code] = seed
        SESSIONS = load_active_sessions_seed()

    sql_event_ids = _load_processed_event_ids_sql()
    legacy_event_ids = _load_processed_event_ids()
    if isinstance(sql_event_ids, list):
        if legacy_event_ids:
            _save_processed_event_ids_sql(legacy_event_ids)
        refreshed_ids = _load_processed_event_ids_sql()
        if isinstance(refreshed_ids, list):
            PROCESSED_EVENT_IDS = refreshed_ids[-20000:]
            PROCESSED_EVENT_ID_SET = set(PROCESSED_EVENT_IDS)

    # Loading performs a one-time JSON-to-SQL seed when the SQL document is absent.
    load_planning_board()
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    .finished-list-table td { padding: 10px 12px; border-bottom: 1px solid #e7edf6; color: #1e293b; font-size: .82rem; vertical-align: middle; }
    .finished-list-table tbody tr:last-child td { border-bottom: 0; }
    .finished-list-table tbody tr:hover { background: #f8fbff; }
    .finished-list-job { min-width: 190px; font-weight: 800; color: #12233f; }
    .finished-list-output { white-space: nowrap; }
    .finished-list-action { text-align: right; white-space: nowrap; }
    .finished-list-action .approve-print-btn { padding: 8px 12px; }
    #finishShiftTab.main-tab-content {
      margin: 0;
      padding: 0;
      border: 0;
      border-radius: 0;
    ...
    #finishShiftTab.main-tab-content .panel {
      padding: 12px 0 0;
      border: 0;
      border-radius: 0;
      background: transparent;
    ...
    #finishShiftTab .panel > h3,
    #finishShiftTab .panel > .muted,
    #finishShiftTab .finish-review-selector,
    #finishShiftTab .finished-view-toolbar {
      margin-left: clamp(10px, 1.6vw, 20px);
      margin-right: clamp(10px, 1.6vw, 20px);
    }
    #finishShiftTab.main-tab-content > .panel > h3 {
      margin-left: clamp(12px, 1.4vw, 22px) !important;
      margin-right: clamp(12px, 1.4vw, 22px) !important;
    }
    #finishShiftTab .finished-list-wrap {
    ...
    #finishShiftTab .finished-list-table {
      min-width: 920px;
    }
    #finishShiftTab .finished-list-table th:first-child,
    #finishShiftTab .finished-list-table td:first-child {
      padding-left: clamp(10px, 1.6vw, 20px);
    }
    #finishShiftTab .finished-list-table th:last-child,
    #finishShiftTab .finished-list-table td:last-child {
      padding-right: clamp(10px, 1.6vw, 20px);
    }
    #finishShiftTab .finished-view-switch {
      border: 0;
      padding: 3px;
    }
    #finishShiftTab .finished-wrap {
      padding: 0 clamp(12px, 1.4vw, 22px) 16px;
    }
    #finishShiftTab .finished-item {
      border: 0;
      box-shadow: none;
    }
    .finished-item {
    ...
    .queue-target-warning { display:inline-block; margin-top:5px; padding:4px 7px; border-radius:7px; background:#fff7ed; color:#c2410c; border:1px solid #fdba74; font-size:.72rem; font-weight:900; line-height:1.25; white-space:nowrap; }
    .queue-target-warning.excess { background:#fef2f2; color:#b91c1c; border-color:#fca5a5; }
    .approve-print-btn {
      border: none;
      border-radius: 10px;
    ...
    #approvePrintOverlay.approved-shift-report .overlay-card { width:min(860px, calc(100vw - 24px)); height:min(850px, calc(100vh - 20px)); max-height:calc(100vh - 20px); border-radius:12px; }
    #approvePrintOverlay.approved-shift-report.linked-shift-report .overlay-card { width:min(1660px, calc(100vw - 24px)); }
    #approvePrintOverlay.approved-shift-report .overlay-head { padding:8px 12px; }
    #approvePrintOverlay.approved-shift-report .overlay-body { padding:8px; overflow:auto; background:#e8edf3; }
    #approvePrintOverlay.approved-shift-report #overlayReviewStep { padding:0; max-height:none; overflow:visible; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide1 > .review-panel:nth-of-type(2),
    #approvePrintOverlay.approved-shift-report #reviewSubslide1 > input,
    #approvePrintOverlay.approved-shift-report #reviewSubslide1 > textarea { display:none !important; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide1 > .review-panel:first-child { border:0; border-radius:0; background:#fff; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide1 > .review-panel:first-child > .review-panel-title { display:none; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide1 > .review-panel:first-child > .review-panel-body { padding:0; background:#fff; overflow:visible; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 > .review-panel { border:0; border-radius:0; background:#fff; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 > .review-panel > .review-panel-title { display:none; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 > .review-panel > .review-panel-body { padding:0; background:#fff; overflow:visible; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 .pdr-sheet { width:min(820px,100%); min-height:680px; margin:0 auto; padding:10px; box-sizing:border-box; box-shadow:0 8px 26px rgba(15,23,42,.12); }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 .pdr-topbar { grid-template-columns:1fr; gap:8px; min-height:72px; padding:12px 16px; box-sizing:border-box; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 .pdr-topbar h2 { font-size:28px; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 .pdr-top-meta { justify-content:flex-start; gap:18px; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 .consumption-report-layout { grid-template-columns:150px minmax(0,1fr); }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 .consumption-content-grid { grid-template-columns:1fr; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 .consumption-content-grid .balance-panel { grid-column:auto; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide3 .consumption-content-grid .review-group-card:nth-child(2) { border-right:0; border-bottom:1px solid #b9c7da; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide5 > .review-panel:first-child { border:0; border-radius:0; background:#fff; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide5 > .review-panel:first-child > .review-panel-title { display:none; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide5 > .review-panel:first-child > .review-panel-body { padding:0; background:#fff; overflow:visible; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide5 > .review-panel:nth-of-type(2) { display:none; }
    #approvePrintOverlay.approved-shift-report #reviewSubslide5 > .review-panel:first-child { width:min(820px,100%); min-height:680px; margin:0 auto; padding:10px; box-sizing:border-box; box-shadow:0 8px 26px rgba(15,23,42,.12); }
    #approvePrintOverlay.approved-shift-report .overlay-actions .review-edge-arrow { position:static; transform:none; width:auto; height:auto; border-radius:14px; padding:9px 20px; font-size:.9rem; font-weight:900; }
    #approvePrintOverlay.approved-shift-report .overlay-actions .review-slide-status { flex:0 0 auto; margin:0 auto 0 0; color:#09266a; font-size:13px; font-weight:950; text-align:left; white-space:nowrap; }
    #approvePrintOverlay.portrait-shift-report .overlay-body { background:#e8edf3; }
    .pdr-sheet { color:#071d50; background:#fff; font-size:12px; line-height:1.2; }
    .pdr-sheet.pdr-portrait { width:min(820px,100%); min-height:680px; margin:0 auto; padding:10px; box-sizing:border-box; box-shadow:0 8px 26px rgba(15,23,42,.12); }
    .linked-shift-layout { width:100%; display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:12px; align-items:start; }
    .linked-shift-layout > .pdr-sheet { width:100%; min-width:0; margin:0; }
    .linked-shift-shell { width:100%; }
    .linked-shift-toolbar { width:min(820px,100%); margin:0 auto 10px; display:flex; justify-content:space-between; align-items:center; gap:10px; }
    .same-job-shift-picker { display:flex; align-items:center; gap:8px; color:#334155; font-size:.8rem; font-weight:900; }
    .same-job-shift-picker select { min-width:300px; max-width:520px; border:1px solid #b9c7da; border-radius:10px; background:#fff; color:#0f172a; padding:8px 10px; font:inherit; cursor:pointer; }
    .linked-shift-layout.linked-hidden { grid-template-columns:minmax(0,820px); justify-content:center; }
    .linked-shift-layout.linked-hidden .linked-job-sheet { display:none; }
    .linked-shift-layout .linked-job-sheet .pdr-info { grid-template-columns:repeat(3,minmax(0,1fr)); }
    .linked-shift-layout .linked-job-sheet .pdr-info-item { grid-column:auto !important; border-right:1px solid #d8e0eb !important; }
    .linked-shift-layout .linked-job-sheet .pdr-info-item:nth-child(3n) { border-right:0 !important; }
    @media (max-width:1100px) {
      .linked-shift-layout { grid-template-columns:1fr; }
    }
    .pdr-portrait .pdr-topbar { grid-template-columns:1fr; gap:8px; min-height:72px; padding:12px 16px; box-sizing:border-box; }
    .pdr-portrait .pdr-topbar h2 { font-size:28px; }
    .pdr-portrait .pdr-top-meta { justify-content:flex-start; gap:18px; }
    .pdr-portrait .pdr-info { grid-template-columns:repeat(5,minmax(0,1fr)); }
    .pdr-portrait .pdr-info-item { min-height:64px; border-right:1px solid #d8e0eb; }
    .pdr-portrait .pdr-info-item:not(.pdr-time-item) { grid-template-columns:1fr; grid-template-rows:auto minmax(0,1fr); }
    .pdr-portrait .pdr-time-item { grid-column:span 2; }
    .pdr-portrait .pdr-info-item:nth-child(11),
    .pdr-portrait .pdr-info-item:nth-child(12) { grid-column:span 2; }
    .pdr-portrait .pdr-info-item:nth-child(4),
    .pdr-portrait .pdr-info-item:nth-child(9),
    .pdr-portrait .pdr-info-item:nth-child(12) { border-right:0; }
    .pdr-portrait .pdr-info-k { align-items:center; min-height:25px; padding:5px 9px; box-sizing:border-box; color:#09266a; background:#edf4fc; font-size:10px; letter-spacing:.025em; text-transform:uppercase; }
    .pdr-portrait .pdr-info-v { align-items:center; padding:7px 9px; color:#071d50; background:#fff; font-size:12px; font-weight:850; }
    .pdr-portrait .pdr-info-item:first-child .pdr-info-v { color:#0756a6; font-size:15px; font-weight:950; }
    .pdr-portrait .pdr-kpis { grid-template-columns:repeat(6,minmax(0,1fr)); }
    .pdr-portrait .pdr-kpi { border-right:1px solid #cbd5e1; }
    .pdr-portrait .pdr-kpi:nth-child(6n) { border-right:0; }
    .pdr-portrait .pdr-kpi .k { min-height:28px; display:flex; align-items:center; justify-content:center; padding:5px 3px; box-sizing:border-box; font-size:8.5px; line-height:1.1; }
    .pdr-portrait .pdr-kpi .v { min-height:42px; display:flex; align-items:center; justify-content:center; padding:6px 3px; box-sizing:border-box; font-size:24px; }
    .pdr-portrait .pdr-reject-grid { min-height:82px; align-content:flex-start; box-sizing:border-box; }
    .pdr-portrait .pdr-detail-grid,
    .pdr-portrait .pdr-detail-grid.single { grid-template-columns:1fr; gap:6px; margin-top:7px; }
    .pdr-portrait .pdr-section-title { padding:6px 10px; }
    .pdr-portrait .pdr-personnel { grid-template-columns:repeat(3,minmax(0,1fr)); margin-top:7px; }
    .pdr-portrait .pdr-person { min-height:62px; padding:8px 9px; display:flex; flex-direction:column; justify-content:center; box-sizing:border-box; }
    .pdr-portrait .pdr-person:nth-child(3n) { border-right:0; }
    .pdr-portrait .pdr-person .k { font-size:11px; line-height:1.2; }
    .pdr-portrait .pdr-person .v { margin-top:6px; font-size:14px; line-height:1.2; font-weight:900; }
    .pdr-topbar { display:grid; grid-template-columns:minmax(360px,1fr) auto; align-items:center; gap:18px; padding:12px 18px; color:#fff; background:linear-gradient(90deg,#031c56,#062c73); border-radius:4px; }
    .pdr-topbar h2 { margin:0; font-size:clamp(22px,2.2vw,34px); letter-spacing:.02em; }
    .pdr-top-meta { display:flex; gap:18px; font-size:13px; font-weight:800; white-space:nowrap; }
    .pdr-info { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); border:1px solid #cbd5e1; border-bottom:0; margin-top:8px; }
    .pdr-info-item { display:grid; grid-template-columns:minmax(115px,.9fr) 1.25fr; min-height:38px; border-right:1px solid #d8e0eb; border-bottom:1px solid #d8e0eb; }
    .pdr-info-item:nth-child(4n) { border-right:0; }
    .pdr-info-k, .pdr-info-v { display:flex; align-items:center; padding:7px 10px; min-width:0; overflow-wrap:anywhere; }
    .pdr-info-k { font-weight:900; color:#09266a; background:#f8fafc; }
    .pdr-info-v { color:#102451; }
    .pdr-time-item { display:grid; grid-template-columns:1fr 1fr; }
    .pdr-time-box { min-width:0; padding:6px 9px; border-right:1px solid #d8e0eb; }
    .pdr-time-box:last-child { border-right:0; }
    .pdr-time-box .k { color:#09266a; font-size:9px; font-weight:900; text-transform:uppercase; }
    .pdr-time-box .v { margin-top:4px; color:#102451; overflow-wrap:anywhere; }
    .pdr-kpis { display:grid; grid-template-columns:repeat(6,minmax(0,1fr)); border:1px solid #b9c7da; margin-top:8px; }
    .pdr-kpi { text-align:center; border-right:1px solid #cbd5e1; }
    .pdr-kpi:last-child { border-right:0; }
    .pdr-kpi .k { padding:8px 5px; color:#fff; background:#052762; font-size:11px; font-weight:900; }
    .pdr-kpi .v { padding:12px 5px; font-size:clamp(20px,2vw,30px); font-weight:950; color:#0a296f; }
    .pdr-kpi.good .v { color:#079455; } .pdr-kpi.butal .v { color:#2563eb; } .pdr-kpi.bad .v { color:#ef1010; } .pdr-kpi.warn .v { color:#f08a00; }
    .consumption-report-layout { display:grid; grid-template-columns:220px minmax(0,1fr); gap:0; align-items:stretch; margin-top:8px; border:1px solid #b9c7da; border-radius:10px; overflow:hidden; background:#fff; }
    .consumption-kpis { display:grid; grid-template-columns:1fr; grid-template-rows:repeat(3,minmax(112px,1fr)); margin:0; border:0; border-right:1px solid #b9c7da; min-width:0; }
    .consumption-kpis .pdr-kpi { display:flex; flex-direction:column; border-right:0; border-bottom:1px solid #cbd5e1; }
    ...
    .scan-history-head { padding:8px 6px; border-bottom:1px solid #dbe4f0; color:#64748b; font-size:.64rem; font-weight:900; letter-spacing:.02em; text-transform:uppercase; }
    .scan-history-item { border-bottom:1px solid #e2e8f0; }
    .scan-history-item:last-child { border-bottom:0; }
    .scan-history-summary { padding:10px 6px; cursor:pointer; list-style:none; color:#0f172a; font-size:.74rem; font-weight:850; }
    .scan-history-head > span, .scan-history-summary > span { min-width:0; overflow-wrap:anywhere; word-break:break-word; }
    .scan-history-summary::-webkit-details-marker { display:none; }
    .scan-history-summary:hover { background:#f8fbff; }
    .scan-history-summary .material { overflow-wrap:anywhere; }
    .scan-history-count { display:inline-flex; margin-top:4px; max-width:100%; padding:2px 6px; border-radius:999px; background:#dbeafe; color:#1d4ed8; font-size:.61rem; line-height:1.2; font-weight:900; white-space:normal; }
    .scan-history-arrow { color:#2563eb; font-size:1rem; text-align:center; transition:transform .18s ease; }
    .scan-history-item[open] .scan-history-arrow { transform:rotate(180deg); }
    .scan-history-body { padding:0 10px 10px 48px; animation:reviewSlideIn .16s ease; }
    .scan-history-body .review-data-table { margin:0; border:1px solid #dbe4f0; border-radius:8px; overflow:hidden; }
    .pdr-detail-grid { display:grid; grid-template-columns:minmax(280px,.75fr) minmax(420px,2fr); gap:10px; margin-top:10px; }
    .pdr-detail-grid.single { grid-template-columns:1fr; }
    .pdr-section { border:1px solid #cbd5e1; min-width:0; }
    .pdr-section-title { padding:8px 12px; color:#fff; background:#052762; font-size:13px; font-weight:950; text-transform:uppercase; }
    .pdr-reject-grid { display:flex; flex-wrap:wrap; gap:6px; padding:6px; }
    .pdr-reject-item { display:inline-flex; align-items:center; justify-content:space-between; gap:14px; min-width:110px; padding:5px 9px; border:1px solid #d8e0eb; border-radius:7px; background:#f8fafc; }
    .pdr-reject-item strong { color:#e11d48; }
    .pdr-empty { padding:12px; color:#64748b; text-align:center; }
    .pdr-downtime-table { width:100%; border-collapse:collapse; }
    .pdr-downtime-table th, .pdr-downtime-table td { padding:8px 10px; border-bottom:1px solid #e0e7f0; text-align:left; }
    .pdr-downtime-table th { color:#09266a; background:#f8fafc; font-size:10px; text-transform:uppercase; }
    .pdr-personnel { display:grid; grid-template-columns:repeat(auto-fit,minmax(170px,1fr)); margin-top:10px; border:1px solid #cbd5e1; border-top:0; }
    .pdr-person { min-height:54px; padding:8px 10px; text-align:center; border-right:1px solid #d8e0eb; border-top:1px solid #d8e0eb; }
    .pdr-person:last-child { border-right:0; }
    .pdr-person .k { font-size:9px; font-weight:900; text-transform:uppercase; color:#09266a; }
    .pdr-person .v { margin-top:8px; color:#102451; font-weight:800; overflow-wrap:anywhere; }
    ...
    .qr-view-btn { border:1px solid #93c5fd; border-radius:8px; background:#eff6ff; color:#1d4ed8; padding:6px 10px; font-size:.74rem; font-weight:900; cursor:pointer; white-space:nowrap; }
    .qr-view-btn:hover { background:#dbeafe; border-color:#60a5fa; }
    .qr-view-card { width:min(860px, calc(100vw - 28px)); max-height:calc(100vh - 28px); background:#fff; border:1px solid #cbd5e1; border-radius:18px; box-shadow:0 24px 60px rgba(15,23,42,.28); overflow:hidden; display:flex; flex-direction:column; }
    .qr-view-head { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:14px 16px; border-bottom:1px solid #e2e8f0; }
    .qr-view-title { color:#0f172a; font-size:1.02rem; font-weight:900; }
    .qr-view-body { display:grid; grid-template-columns:minmax(260px, 340px) minmax(0,1fr); gap:18px; align-items:start; padding:18px; overflow:auto; }
    .qr-view-image { display:flex; align-items:center; justify-content:center; min-height:300px; border:1px solid #dbe4f0; border-radius:14px; background:#fff; padding:12px; }
    .qr-view-image img { display:block; width:100%; max-width:320px; height:auto; image-rendering:pixelated; }
    .qr-view-image .machine-detail-empty { text-align:center; }
    .qr-view-details { border:1px solid #dbe4f0; border-radius:14px; overflow:hidden; background:#f8fafc; }
    .qr-view-details .review-kv-table { background:#fff; }
    .qr-view-payload { margin:12px; padding:10px; max-height:180px; overflow:auto; border:1px solid #dbe4f0; border-radius:10px; background:#f8fafc; color:#0f172a; font:600 12px/1.45 "Consolas","Courier New",monospace; white-space:pre-wrap; overflow-wrap:anywhere; }
    ...
    /* Shared full-width, borderless tab layout with a readable data gutter. */
    #machinesTab.main-tab-content,
    #planningTab.main-tab-content,
    #finishShiftTab.main-tab-content,
    #archivedJobsTab.main-tab-content,
    #machineArchiveTab.main-tab-content,
    #maintenanceTab.main-tab-content,
    #userKpiTab.main-tab-content,
    #pdrTab.main-tab-content {
      margin: 0 !important;
      padding: 12px clamp(12px, 1.4vw, 22px) 18px !important;
      border: 0 !important;
      border-radius: 0 !important;
      background: transparent !important;
      box-shadow: none !important;
    }
    #finishShiftTab > .panel,
    #archivedJobsTab > .panel,
    #machineArchiveTab > .panel,
    #pdrTab > .panel,
    #planningTab > .planning-shell,
    #maintenanceTab > .maintenance-shell,
    #userKpiTab > .user-kpi-shell {
      padding: 0 !important;
      border: 0 !important;
      border-radius: 0 !important;
      background: transparent !important;
    ...
    #archivedJobsTab .table-wrap,
    #machineArchiveTab .table-wrap,
    #pdrTab .table-wrap {
      border-left: 0;
      border-right: 0;
      border-radius: 0;
    ...
    @media (max-width: 640px) {
      ...
      #additionalMachineGrid { grid-template-columns:repeat(2, minmax(0, 1fr)); }
      ...
      .qr-view-body { grid-template-columns:1fr; }
      .pdr-topbar, .pdr-info, .pdr-detail-grid, .pdr-detail-grid.single, .pdr-personnel { grid-template-columns:1fr; }
      .pdr-top-meta { flex-wrap:wrap; white-space:normal; }
      .pdr-kpis { grid-template-columns:repeat(2,minmax(0,1fr)); }
      .consumption-report-layout { grid-template-columns:1fr; }
      .consumption-kpis { grid-template-columns:repeat(3,minmax(0,1fr)); grid-template-rows:1fr; }
      .consumption-kpis { border-right:0; border-bottom:1px solid #b9c7da; }
      .consumption-kpis .pdr-kpi { border-right:1px solid #cbd5e1; border-bottom:0; }
      .consumption-kpis .pdr-kpi:last-child { border-right:0; }
      .consumption-content-grid { grid-template-columns:1fr; }
      .consumption-content-grid .balance-panel { grid-column:auto; }
      .consumption-content-grid .review-group-card:nth-child(2) { border-right:0; border-bottom:1px solid #b9c7da; }
    ...
    @media (max-width: 420px) {
      ...
      #additionalMachineGrid { grid-template-columns:1fr; }
...
<body>
  ...
  <div class="main-tabs">
    ...
    <button class="main-tab-button" data-target="finishShiftTab">Finish Shift Review</button>
  ...
  <div id="machinesTab" class="main-tab-content active">
    ...
    <div class="additional-machines-section">
      <div class="additional-machines-title">Factory 4 Machines</div>
      <div class="grid" id="additionalMachineGrid"></div>
    </div>
  </div>
    ...
    <div class="panel">
      ...
      <div class="finish-review-selector">
        ...
        <select id="finishShiftViewSelect">
          <option value="finishShiftQueuePane">Finished Shifts</option>
          <option value="finishShiftProgressPane">Job Progress</option>
          <option value="finishShiftJobsPane">Finished Jobs</option>
        </select>
      </div>
      ...
      <div id="finishShiftJobsPane" class="sub-tab-content">
        <div class="finish-review-pane-head">
  ...
  <div id="qrViewOverlay" class="settings-overlay" role="dialog" aria-modal="true" aria-labelledby="qrViewTitle">
    <div class="qr-view-card">
      <div class="qr-view-head">
        <div id="qrViewTitle" class="qr-view-title">QR Details</div>
        <button id="qrViewCloseBtn" class="overlay-close" type="button" aria-label="Close QR viewer">Close</button>
      </div>
      <div class="qr-view-body">
        <div id="qrViewImage" class="qr-view-image"></div>
        <div id="qrViewDetails" class="qr-view-details"></div>
      </div>
    </div>
  </div>
  ...
  <div id="approvePrintOverlay" class="overlay-backdrop">
    <div class="overlay-card">
      <div class="overlay-head">
        <div id="approvePrintOverlayTitle" class="overlay-title">Approve and Print QR</div>
        ...
        <div class="settings-content">
          ...
          <div id="settingsPageMachines" class="settings-page">
            <div class="settings-form">
              ...
              <div class="settings-note" style="margin-top:18px;"><strong>Clear Machine Data</strong><br />Remove the selected machine's current active session and counters from the server. Finished-job history is not deleted.</div>
              <div class="settings-row">
                <label>Machine with active data</label>
                <select id="settingsMachineClearSelect">
                  <option value="">No active machine data</option>
                </select>
              </div>
              <div class="settings-actions">
          ...
          <div id="settingsPageClients" class="settings-page">
            <div class="settings-form">
              <div class="settings-note">Clients detected by the server. The IP address is updated whenever a client sends an event.</div>
              <div class="settings-actions">
                <button id="settingsClientsRefreshBtn" class="btn-secondary" type="button">Refresh Clients</button>
              </div>
          ...
          <div id="settingsPageProfile" class="settings-page">
            <div class="settings-form">
              ...
              <div class="settings-row">
                ...
                <input id="settingsProfilesCount" type="text" readonly value="Loading..." />
              </div>
              <div class="settings-actions">
                <button id="settingsProfilesRefreshBtn" class="btn-secondary" type="button">Refresh Profiles</button>
              </div>
              <div class="settings-table-wrap">
                <table class="settings-table">
        ...
        <div class="settings-form">
          ...
          <div class="settings-row">
            ...
            <select id="dailyRoleExtraPrivilegeSelect">
              ...
              <option value="maintenance">Add Maintenance Privilege</option>
              <option value="operator">Add Operator Privilege</option>
            </select>
          </div>
          <div class="settings-row">
...
<script>
  ...
  const finishedShiftListViewBtn = document.getElementById("finishedShiftListViewBtn");
  ...
  const ADDITIONAL_MACHINE_CODES = new Set([
    "M00111", "M00112", "M00113", "M00114", "M00115", "M00116", "M00117", "M00118",
    "M00121", "M00122", "M00123", "M00124", "M00125", "M00126",
    "M00141", "M00142", "M00143", "M00144",
  ]);
  ...
  let finishedShiftViewMode = (() => {
    try {
      return localStorage.getItem("finishedShiftViewMode") === "list" ? "list" : "grid";
    } catch(_err) {
      return "grid";
    }
  })();
    ...
    return `
      ...
      <div class="review-group-list">
        ...
        <div class="review-group-card wide">
          <div class="review-group-head">Raw Material Scanned QR History</div>
          <div class="review-group-body">${rawMaterialQrHistoryHtml(rawRows)}</div>
        </div>
        <div class="review-group-card wide">
          ...
          <div class="review-group-body">${tableFromRows(packRows, [
            ...
            { label: "QR", html: x => qrViewButton(qrPackDetails(x)) },
  ...
  function parsedRawMaterialScan(payload){
    ...
    return {
      product_id: numeric(match[1]),
      qty: numeric(match[2]),
      index: numeric(match[3]),
      total: numeric(match[4]),
      lot: match[5],
    };
  }
  ...
  function archivedMaterialRows(session){
    ...
    for(let i = 0; i < max; i += 1){
      ...
      const explicitScanPayload = firstValue(
        logObj.raw_scan, logObj.qr_payload, logObj.payload, logObj.scan_value,
        scanObj.raw_scan, scanObj.qr_payload, scanObj.payload, scanObj.scan_value
      );
      const legacyScanPayload = scanText.length >= 40 ? scanText : "";
      const scanPayload = firstValue(explicitScanPayload, legacyScanPayload);
      const scanTime = firstValue(logObj.timestamp_utc, logObj.scanned_at, scanObj.timestamp_utc, scanObj.scanned_at);
      const scanQty = firstValue(logObj.qty, logObj.quantity, scanObj.qty, scanObj.quantity);
      const hasScanRecord = String(scanPayload || "").trim() !== "";
      const parsedScan = parsedRawMaterialScan(scanPayload);
      rows.push({
        index: i + 1,
        material: firstValue(
        ...
        time: scanTime || "",
        code: firstValue(logObj.material_code, logObj.product_id, logObj.sku, scanObj.material_code, scanObj.product_id, scanObj.sku, ""),
        raw_scan: String(scanPayload || ""),
        is_scanned: hasScanRecord,
        scan_qty: firstValue(parsedScan.qty, scanQty, "-"),
        scan_index: firstValue(parsedScan.index, logObj.index, scanObj.index, i + 1),
        scan_lot: firstValue(parsedScan.lot, logObj.lot_number, logObj.lot, scanObj.lot_number, scanObj.lot, "-"),
        source: firstValue(logObj.source, scanObj.source, ""),
        carryover_from_job_code: firstValue(logObj.carryover_from_job_code, scanObj.carryover_from_job_code, ""),
        carried_at_utc: firstValue(logObj.carried_at_utc, scanObj.carried_at_utc, ""),
      });
    }
    return rows;
  }
  ...
  function jobPartsNeededHtml(row){
    const item = (row && typeof row === "object") ? row : {};
    const sources = [{
      label: `Main Job - ${firstValue(item.job_name, jobDisplayName(item, "-"))}`,
      row: item,
    }];
    const linkedRows = Array.isArray(item.linkage_jobs) ? item.linkage_jobs : [];
    linkedRows.forEach(linked => {
      if(!linked || typeof linked !== "object") return;
      const linkedCode = firstValue(linked.job_code, linked.job_name, "-");
      let linkedSource = linked;
      const hasPayload = linked.job_payload && typeof linked.job_payload === "object" && Object.keys(linked.job_payload).length;
      if(!hasPayload && String(linkedCode) === String(item.linkage_job_code || "") && item.linkage_job_payload && typeof item.linkage_job_payload === "object"){
        linkedSource = {...linked, job_payload: item.linkage_job_payload};
      }
      sources.push({
        label: `Linked Job - ${firstValue(linkedSource.job_name, jobDisplayName(linkedSource, "-"))}`,
        row: linkedSource,
      });
    });
    const partRows = [];
    sources.forEach(source => {
      const seen = new Set();
      rawPartRows(source.row).forEach(part => {
        const productId = firstValue(part.part_product_id, part.product_id, part.id, "-");
        const sku = firstValue(part.sku, part.part_sku, part.product_sku, part.part_code, part.code, "-");
        const key = `${String(productId).trim()}|${String(sku).trim().toUpperCase()}`;
        if(seen.has(key)) return;
        seen.add(key);
        partRows.push({
          required_by: source.label,
    ...
    partRows.forEach((part, idx) => { part.index = idx + 1; });
    return tableFromRows(partRows, [
      { label: "#", value: x => x.index },
      { label: "Required By", value: x => x.required_by },
      { label: "Product ID", value: x => x.product_id },
      { label: "SKU", value: x => x.sku },
      { label: "Part Needed", value: x => x.name },
      { label: "Qty / Unit", value: x => x.per_unit },
      { label: "Requested", value: x => x.requested },
      { label: "Approved", value: x => x.approved },
      { label: "Completed", value: x => x.completed },
    ], "No required-parts list was returned by the Job API.", Math.max(1, partRows.length));
  }
  ...
  function rawMaterialInsightsHtml(row){
    ...
    return `
      ...
      <div class="review-group-list">
        ...
        <div class="review-group-card wide">
          <div class="review-group-head">Raw Material Scanned QR History</div>
          <div class="review-group-body">${rawMaterialQrHistoryHtml(materialRows)}</div>
        </div>
        <div class="review-group-card wide">
  ...
  function openMachineDetail(session){
    ...
    const downtimeHistory = Array.isArray(session.pdr_downtime_logs)
      ? session.pdr_downtime_logs.filter(row => row && typeof row === "object")
      : [];
    const activeDowntimeSeconds = session.downtime_active && session.downtime_started_at
      ? Math.max(0, Number(session.downtime_last_seconds || 0))
        + Math.max(0, Math.floor(Date.now() / 1000 - Number(session.downtime_started_at || 0)))
      : 0;
    const totalDowntimeSeconds = downtimeHistory.reduce(
      (sum, row) => sum + Math.max(0, Number(row.duration_seconds || 0)), 0
    ) + activeDowntimeSeconds;
    const downtimeHistoryRows = downtimeHistory.map((row, idx) => {
      const segments = Array.isArray(row.reason_segments) ? row.reason_segments : [];
      const reasons = segments.length
        ? segments.map(segment => `${segment.reason_code || ""} ${segment.reason || ""}`.trim()).join(" -> ")
        : `${row.reason_code || ""} ${row.reason || "-"}`.trim();
      return {
        index: idx + 1,
        reasons,
        ...
        finished: fmtDateLocal(row.finished_at_utc || ""),
      };
    });
    const counterOverwriteRows = (Array.isArray(session.machine_counter_overwrite_logs)
      ? session.machine_counter_overwrite_logs
      : []
    ).filter(row => row && typeof row === "object").map((row, idx) => ({
      index: idx + 1,
      old_value: row.old_value ?? "-",
      new_value: row.new_value ?? "-",
      supervisor: row.supervisor_name || row.supervisor_code || "-",
      changed_at: fmtDateLocal(row.changed_at_utc || ""),
      job: row.job_name || row.job_code || "-",
      operator: displayNameForId(row.operator_id || "-"),
    }));
    const cycleTimeChangeRows = (Array.isArray(session.cycle_time_change_logs)
      ? session.cycle_time_change_logs
      : []
    ).filter(row => row && typeof row === "object").map((row, idx) => ({
      index: idx + 1,
      old_value: row.previous_cycle_time ?? "-",
      new_value: row.cycle_time ?? row.cycle_seconds ?? "-",
      supervisor: row.confirmed_by || row.confirmed_by_code || "-",
      role: row.confirmed_by_role || "-",
      changed_at: fmtDateLocal(row.set_at_utc || ""),
      source: row.source || "-",
    }));
    ...
    machineDetailBody.innerHTML = `
      ...
      <div class="machine-detail-section">
        <h4>Parts Needed for Job</h4>
        ${jobPartsNeededHtml(session)}
      </div>
      <div class="machine-detail-section">
        <h4>Machine Counter Overwrite Audit</h4>
        ${tableFromRows(counterOverwriteRows, [
          ["#", "index"],
          ["Old Counter", "old_value"],
          ["New Counter", "new_value"],
          ["Supervisor", "supervisor"],
          ["Changed At", "changed_at"],
          ["Job", "job"],
          ["Operator", "operator"],
        ], "No machine counter overwrite recorded for this shift.")}
      </div>
      <div class="machine-detail-section">
        <h4>Cycle Time Change History</h4>
        ${tableFromRows(cycleTimeChangeRows, [
          ["#", "index"],
          ["Old", "old_value"],
          ["Current", "new_value"],
          ["Supervisor", "supervisor"],
          ...
          ["Changed At", "changed_at"],
          ["Source", "source"],
        ], "No cycle-time change recorded for this shift.")}
      </div>
      <div class="machine-detail-section">
        <h4>Product Pack History</h4>
        ${productPackHistoryHtml(session, 12)}
      ...
      <div class="machine-detail-section">
        <h4>Downtime</h4>
        <div class="machine-detail-grid">
          ${detailItem("Total Downtime", fmtDowntimeSeconds(totalDowntimeSeconds))}
          ${detailItem("Occurrences", downtimeHistory.length)}
          ${detailItem("Reason Code", session.downtime_reason_code || "-")}
          ${detailItem("Reason", session.downtime_reason_text || "-")}
          ${detailItem("Current/Last Duration", fmtDowntimeSeconds(session.downtime_active ? activeDowntimeSeconds : session.downtime_last_seconds))}
          ${detailItem("Downtime Start", session.downtime_started_at ? fmtDateLocal(Number(session.downtime_started_at) * 1000) : "-")}
        </div>
        ${tableFromRows(downtimeHistoryRows, [
          ["#", "index"],
          ["Reason(s)", "reasons"],
          ["Duration", "duration"],
          ...
          ["Finished", "finished"],
        ], "No completed downtime recorded for this shift.")}
      </div>
      <div class="machine-detail-section">
  ...
  function renderMachinePicker(){
    ...
    const standardCodes = DEFAULT_MACHINE_CODES.filter(code => !ADDITIONAL_MACHINE_CODES.has(code));
    const additionalCodes = DEFAULT_MACHINE_CODES.filter(code => ADDITIONAL_MACHINE_CODES.has(code));
    const renderItems = codes => codes.map(code => {
  ...
  async function loadSettingsProfilesUi(){
    try {
      ...
      if(settingsProfilesCount) settingsProfilesCount.value = `${rows.length} profile${rows.length === 1 ? "" : "s"}`;
      ...
      if(settingsProfilesCount) settingsProfilesCount.value = "Unable to load";
  ...
  function renderMachineDataClearer(){
    if(!settingsMachineClearSelect) return;
    const previous = String(settingsMachineClearSelect.value || "").trim();
    const sessions = Array.isArray(latestState?.sessions) ? latestState.sessions : [];
    const rows = sessions
      .filter(s => String(s?.machine_code || "").trim())
      .sort((a, b) => String(a.machine_code || "").localeCompare(String(b.machine_code || "")));
    settingsMachineClearSelect.innerHTML = "";
    const placeholder = document.createElement("option");
    placeholder.value = "";
    placeholder.textContent = rows.length ? "Select a machine..." : "No active machine data";
    settingsMachineClearSelect.appendChild(placeholder);
    rows.forEach(s => {
      const code = String(s.machine_code || "").trim();
      const name = String(s.machine_name || MACHINE_NAME_MAP[code] || code).trim();
      const job = String(s.job_code || "").trim();
    ...
    if(previous && rows.some(s => String(s.machine_code || "").trim() === previous)){
      settingsMachineClearSelect.value = previous;
    }
    if(settingsMachineClearBtn) settingsMachineClearBtn.disabled = !settingsMachineClearSelect.value;
  }

  async function clearSelectedMachineData(){
    const code = String(settingsMachineClearSelect?.value || "").trim();
    if(!code) return;
    const option = settingsMachineClearSelect?.selectedOptions?.[0];
    const clientId = String(option?.dataset?.clientId || "").trim();
    const label = String(option?.textContent || code).trim();
    if(!clientId){
    ...
    if(settingsMachineClearStatus) settingsMachineClearStatus.textContent = "Clearing active machine data...";
    try {
      const resp = await fetch("/api/active-sessions/discard", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({ machine_code: code, client_id: clientId }),
      });
      const out = await resp.json().catch(() => ({}));
      if(!resp.ok || !out.ok) throw new Error(out.error || `Server returned ${resp.status}`);
      latestState.sessions = (latestState.sessions || []).filter(s => String(s.machine_code || "").trim() !== code);
      render(latestState);
      renderMachineDataClearer();
      if(settingsMachineClearStatus) settingsMachineClearStatus.textContent = out.discarded ? `${label} active data was cleared.` : `${label} had no active data to clear.`;
    } catch(err) {
      if(settingsMachineClearStatus) settingsMachineClearStatus.textContent = `Could not clear machine data: ${err?.message || err}`;
    } finally {
      if(settingsMachineClearBtn){
        settingsMachineClearBtn.textContent = "Clear Selected Machine Data";
        settingsMachineClearBtn.disabled = !settingsMachineClearSelect?.value;
      }
    }
  }

  async function loadSettingsClientsUi(){
    if(settingsClientsTableBody) settingsClientsTableBody.innerHTML = `<tr><td colspan="5">Loading clients...</td></tr>`;
    try {
      const resp = await fetch("/api/client-status");
      const out = await resp.json();
      const rows = Array.isArray(out.items) ? out.items : [];
      if(!settingsClientsTableBody) return;
      settingsClientsTableBody.innerHTML = rows.length ? rows.map(row => {
        const seenMs = Date.parse(row.last_seen_utc || "");
        const online = Number.isFinite(seenMs) && (Date.now() - seenMs) <= 120000;
        const machine = row.machine_name || row.machine_code || "-";
        const job = String(row.job_code || "").trim();
        ...
        return `<tr>
          ...
          <td>${esc(machine)}</td>
          <td>${esc(fmtDateLocal(row.last_seen_utc || ""))}</td>
          <td><span class="status-dot ${online ? "connected" : "disconnected"}"></span>${esc(statusLabel)}</td>
        </tr>`;
  ...
  function setOverlayStep(step){
    ...
    approvePrintOverlay?.classList.toggle("approved-shift-report", approvedShiftView);
    ...
    overlayCancelBtn.style.display = approvedShiftView ? "none" : "";
    syncReviewSubslides();
  }
  ...
  function syncReviewSubslides(){
    ...
    const visibleSlides = approvedShiftView
      ? slides.filter((el, idx) => el && (idx === 0 || idx === 2))
      : slides.filter((el, idx) => el && !(overlayReviewMode !== "shift" && idx === 1));
    ...
    approvePrintOverlay?.classList.toggle(
      "portrait-shift-report",
      Boolean(approvedShiftView && active === reviewSubslide1)
    );
    ...
    if(overlayReviewSlideStatus){
      const labels = approvedShiftView
        ? ["Shift Report", "Product Parts and Consumption"]
        : overlayReviewMode === "shift"
    ...
    const toolbar = document.querySelector("#overlayReviewStep .review-slide-toolbar");
    const actions = approvePrintOverlay?.querySelector(".overlay-actions");
    if(approvedShiftView){
      if(toolbar) toolbar.style.display = "none";
      if(actions && overlayReviewPrevBtn && overlayReviewNextBtn){
        overlayReviewPrevBtn.textContent = "Previous";
        overlayReviewNextBtn.textContent = "Next";
        if(overlayReviewSlideStatus) actions.insertBefore(overlayReviewSlideStatus, actions.firstChild);
        actions.appendChild(overlayReviewPrevBtn);
        actions.appendChild(overlayReviewNextBtn);
      }
    } else if(toolbar){
  ...
  function tableFromRows(rows, columns, emptyLabel = "No data.", maxRows = 8){
    ...
    const cols = (Array.isArray(columns) ? columns : []).map(col => {
      if(Array.isArray(col)){
        ...
        return { label, value: item => item?.[field] };
      }
      return col && typeof col === "object" ? col : { label: "", value: () => "-" };
    });
      ...
      <table class="review-kv-table review-data-table">
        ...
        <tbody>
          ${visible.map(item => `
            <tr>
              ${cols.map(col => {
                if(typeof col.html === "function") return `<td>${col.html(item)}</td>`;
                const value = typeof col.value === "function"
                  ? col.value(item)
                  : (typeof col.value === "string" ? item?.[col.value] : col.value);
                return `<td>${esc(compactValue(value))}</td>`;
              }).join("")}
            </tr>
          `).join("")}
        </tbody>
  ...
  function qrViewButton(details, label = "View QR"){
    const encoded = encodeURIComponent(JSON.stringify(details || {}));
    return `<button type="button" class="qr-view-btn" data-qr-view="${esc(encoded)}">${esc(label)}</button>`;
  }

  function qrPartDetails(part, index, session){
    const x = (part && typeof part === "object") ? part : {};
    const product = firstValue(x.part_name, x.product_name, x.name, x.item_name, x.description, "-");
    const code = firstValue(x.part_code, x.product_code, x.code, x.sku, x.product_id, "-");
    const qty = firstValue(x.qty, x.quantity, x.required_qty, x.request_part_qty, "-");
    const timestamp = firstValue(x.timestamp_utc, x.created_at, x.updated_at, session?.ended_at_utc, session?.finished_at_utc, "");
    const lotNumber = firstValue(x.lot_number, x.lot, session?.lot_number, "-");
    const suppliedPayload = firstValue(x.qr_payload, x.raw_scan, x.payload, x.value, "");
    ...
    return {
      title: "Product Part QR",
      payload: suppliedPayload || generatedPayload,
      product,
      code,
      index,
      qty,
  ...
  function qrPackDetails(row){
    const x = (row && typeof row === "object") ? row : {};
    const payload = String(x.raw_scan || "").trim() || JSON.stringify({
      product: x.product || null,
      index: x.series === "-" ? x.index : x.series,
      qty: x.qty,
      timestamp: x.time || null,
      lot_number: x.lot === "-" ? null : x.lot,
      po_number: x.po === "-" ? null : x.po,
    });
    return {
      title: "Scanned Pack QR",
      payload,
      product: x.product,
      index: x.series === "-" ? x.index : x.series,
      qty: x.qty,
  ...
  function qrRawMaterialDetails(row){
    const x = (row && typeof row === "object") ? row : {};
    const payload = String(x.raw_scan || "").trim() || JSON.stringify({
      product: x.material || null,
      code: x.code || null,
      index: firstValue(x.scan_index, x.index),
      qty: firstValue(x.scan_qty, x.qty),
      timestamp: x.time || null,
      lot_number: firstValue(x.scan_lot, x.lot) === "-" ? null : firstValue(x.scan_lot, x.lot),
    });
    return {
      title: "Scanned Raw Material QR",
      payload,
      product: x.material,
      code: x.code,
      index: firstValue(x.scan_index, x.index),
      qty: firstValue(x.scan_qty, x.qty),
      timestamp: x.time,
      lot_number: firstValue(x.scan_lot, x.lot),
    };
  }

  function rawMaterialQrHistoryHtml(rows){
    const items = (Array.isArray(rows) ? rows : [])
      .filter(x => x?.is_scanned === true)
      .map((x, idx) => ({...x, history_index: idx + 1}));
    return tableFromRows(items, [
      { label: "#", value: x => x.history_index },
      { label: "Material", value: x => x.material },
      { label: "Qty", value: x => firstValue(x.scan_qty, x.qty) },
      { label: "Lot", value: x => firstValue(x.scan_lot, x.lot) },
      { label: "Scanned At", value: x => fmtDateLocal(x.time || "") || "-" },
      { label: "QR", html: x => qrViewButton(qrRawMaterialDetails(x)) },
    ], "No raw-material QR scans recorded.", Math.max(1, items.length));
  }

  function productPartQrHistoryHtml(parts, session){
    const items = Array.isArray(parts) ? parts : [];
    return tableFromRows(items, [
      { label: "#", value: x => items.indexOf(x) + 1 },
      ...
      { label: "QR", html: x => qrViewButton(qrPartDetails(x, items.indexOf(x) + 1, session)) },
    ], "No product-part QR records.", Math.max(1, items.length));
  }

  async function openQrViewer(details){
    const overlay = document.getElementById("qrViewOverlay");
    const title = document.getElementById("qrViewTitle");
    const image = document.getElementById("qrViewImage");
    const detail = document.getElementById("qrViewDetails");
    if(!overlay || !image || !detail) return;
    const info = (details && typeof details === "object") ? details : {};
    const payload = String(info.payload || "");
    title.textContent = info.title || "QR Details";
    image.innerHTML = `<div class="machine-detail-empty">Creating QR&hellip;</div>`;
    detail.innerHTML = `
      ${renderKeyValueTableHtml({
        product: info.product || "-",
        index: info.index ?? "-",
        qty: info.qty ?? "-",
        timestamp: fmtDateLocal(info.timestamp || "") || info.timestamp || "-",
        lot_number: info.lot_number || "-",
        po_number: info.po_number || undefined,
        code: info.code || undefined,
      })}
      <div class="qr-view-payload">${esc(payload || "-")}</div>
    `;
    overlay.classList.add("active");
    try {
      const response = await fetch("/api/qr-view", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({payload}),
      });
      const out = await response.json();
      if(!response.ok || !out.ok || !out.image_data_url) throw new Error(out.error || "QR unavailable");
      image.innerHTML = `<img src="${esc(out.image_data_url)}" alt="Scannable QR code" />`;
    } catch(err) {
      image.innerHTML = `<div class="machine-detail-empty">Unable to create this QR.<br>${esc(err?.message || String(err))}</div>`;
    }
  }
  ...
  function shiftDailyReportHtml(row){
    const item = (row && typeof row === "object") ? row : {};
    const payload = (item.job_payload && typeof item.job_payload === "object") ? item.job_payload : {};
    const data = (payload.data && typeof payload.data === "object") ? payload.data : {};
    const job = (data.job && typeof data.job === "object") ? data.job : {};
    const details = (data.job_details && typeof data.job_details === "object") ? data.job_details : {};
    const rejectRows = Object.entries((item.reject_breakdown && typeof item.reject_breakdown === "object") ? item.reject_breakdown : {})
      .map(([reason, qty]) => ({reason, qty:Number(qty || 0)}))
      .filter(x => x.qty > 0)
      .sort((a,b) => b.qty - a.qty);
    const downtimeSeconds = Math.max(0, Number(item.downtime_last_seconds || 0));
    const downtimeCode = String(item.downtime_reason_code || "").trim();
    const downtimeText = String(item.downtime_reason_text || "").trim();
    const meaningfulDowntime = value => !["", "-", "NONE", "N/A", "NO DOWNTIME"].includes(String(value || "").trim().toUpperCase());
    const hasDowntime = downtimeSeconds > 0 || meaningfulDowntime(downtimeCode) || meaningfulDowntime(downtimeText);
    const good = Math.max(0, Number(item.good_total || 0));
    const butal = Math.max(0, Number(item.butal_total || 0));
    const productionReject = Math.max(0, Number(item.reject_total || 0));
    const startupReject = Math.max(0, Number(item.startup_reject_total || 0));
    const totalReject = productionReject + startupReject;
    const totalMade = good + butal + totalReject;
    const rejectRate = totalMade > 0 ? `${((totalReject / totalMade) * 100).toFixed(2)}%` : "0.00%";
    const started = item.started_at_utc || item.job_started_at || "";
    const ended = item.ended_at_utc || item.finished_at_utc || "";
    const endedDate = ended ? new Date(ended) : null;
    const reportDate = endedDate && !Number.isNaN(endedDate.getTime())
      ? endedDate.toLocaleDateString("en-US", {year:"numeric", month:"2-digit", day:"2-digit"})
      : "-";
    const reportDay = endedDate && !Number.isNaN(endedDate.getTime())
      ? endedDate.toLocaleDateString([], {weekday:"short"}).toUpperCase()
      : "-";
    const infoRows = [
      ["Machine No.", item.machine_name || item.machine_code || "-"],
      ["Work Order No.", job.ref_no || job.code || item.job_code || "-"],
      ["__TIME_SPLIT__", {started:fmtDateLocal(started) || "-", ended:fmtDateLocal(ended) || "-"}],
      ["Mold / Description", details.mold || details.mold_no || job.custom_05 || "-"],
      ["Color", details.color || job.custom_06 || "-"],
      ["Target / Shift", details.qty_per_shift || job.custom_11 || job.request_qty || "-"],
      ["System Code", item.product_sku || job.product_sku || job.sku || "-"],
      ["Cycle Time", item.cycle_time_current || details.std_cycle_time || job.custom_09 || "-"],
      ["Actual Output", item.total_good ?? item.partial_qty ?? (good + butal)],
      ["No. of Cavities", details.no_of_cavity || "-"],
    ...
    const personnelRows = [
      ...
      ["Prepared By: Machine Operator", displayNameForId(item.operator_id || item.operator_name || "-")],
      ["Checked By: Production Supervisor", item.supervisor_name || item.approved_by || "-"],
    ];
    const rejectHtml = rejectRows.length ? `
      <section class="pdr-section">
        <div class="pdr-section-title">Reject Details</div>
        <div class="pdr-reject-grid">
          ${rejectRows.map(x => `<div class="pdr-reject-item"><span>${esc(x.reason)}</span><strong>${esc(x.qty)}</strong></div>`).join("")}
        </div>
      </section>
    ` : "";
    const downtimeHtml = hasDowntime ? `
      <section class="pdr-section">
        <div class="pdr-section-title">Downtime</div>
        <table class="pdr-downtime-table">
          <thead><tr><th>No.</th><th>Description</th><th>Duration</th><th>Remarks</th></tr></thead>
          <tbody><tr><td>01</td><td>${esc(downtimeText || downtimeCode || "Downtime")}</td><td>${esc(fmtDowntimeSeconds(downtimeSeconds))}</td><td>${esc(downtimeCode && downtimeText && downtimeCode !== downtimeText ? downtimeCode : "-")}</td></tr></tbody>
        </table>
      </section>
    ` : "";
    const detailSections = [rejectHtml, downtimeHtml].filter(Boolean);
      ...
      <div class="pdr-sheet pdr-portrait">
        ...
        <div class="pdr-info">${infoRows.map(([key,value]) => key === "__TIME_SPLIT__"
          ? `<div class="pdr-info-item pdr-time-item">
              <div class="pdr-time-box"><div class="k">Time Started</div><div class="v">${esc(value.started)}</div></div>
              <div class="pdr-time-box"><div class="k">Time Ended</div><div class="v">${esc(value.ended)}</div></div>
            </div>`
          : `<div class="pdr-info-item"><div class="pdr-info-k">${esc(key)}</div><div class="pdr-info-v">${esc(value)}</div></div>`).join("")}</div>
        <div class="pdr-kpis">
          <div class="pdr-kpi good"><div class="k">GOOD OUTPUT</div><div class="v">${esc(good)}</div></div>
          <div class="pdr-kpi butal"><div class="k">BUTAL</div><div class="v">${esc(butal)}</div></div>
          <div class="pdr-kpi bad"><div class="k">PRODUCTION REJECT</div><div class="v">${esc(productionReject)}</div></div>
          <div class="pdr-kpi warn"><div class="k">START-UP REJECT</div><div class="v">${esc(startupReject)}</div></div>
          <div class="pdr-kpi"><div class="k">TOTAL REJECT</div><div class="v">${esc(totalReject)}</div></div>
          <div class="pdr-kpi"><div class="k">REJECT RATE</div><div class="v">${esc(rejectRate)}</div></div>
        </div>
        ${detailSections.length ? `<div class="pdr-detail-grid${detailSections.length === 1 ? " single" : ""}">${detailSections.join("")}</div>` : ""}
        ${personnelRows.length ? `<section class="pdr-section" style="margin-top:10px;">
          <div class="pdr-section-title">Shift Personnel</div>
          <div class="pdr-personnel" style="margin-top:0;border-left:0;border-right:0;border-bottom:0;">
            ${personnelRows.map(([key,value]) => `<div class="pdr-person"><div class="k">${esc(key)}</div><div class="v">${esc(value)}</div></div>`).join("")}
          </div>
        </section>` : ""}
      </div>
    `;
    return (linkedJobs.length || sameJobShifts.length > 1) ? `
      <div class="linked-shift-shell">
        <div class="linked-shift-toolbar">
          ${shiftPicker || "<span></span>"}
          ${linkedJobs.length ? `<button class="approve-print-btn toggle-linked-job-btn" type="button" aria-expanded="false">Show Linked Job${linkedJobs.length > 1 ? "s" : ""}</button>` : ""}
        </div>
        ${linkedJobs.length ? `
          <div class="linked-shift-layout linked-hidden">
            ${mainReport}
            ${linkedJobs.map(linked => linkedJobReportHtml(linked, item)).join("")}
          </div>
        ` : mainReport}
      </div>
    ` : mainReport;
  ...
  function linkedJobReportHtml(linked, shift){
    const item = (linked && typeof linked === "object") ? linked : {};
    const payload = (item.job_payload && typeof item.job_payload === "object") ? item.job_payload : {};
    const data = (payload.data && typeof payload.data === "object") ? payload.data : {};
    const job = (data.job && typeof data.job === "object") ? data.job : {};
    const details = (data.job_details && typeof data.job_details === "object") ? data.job_details : {};
    const parts = rawPartRows(item);
    const materialRows = archivedMaterialRows(shift);
    const producedUnits = consumedUnitCount(shift) || Number(shift.total_good || 0) || (Number(shift.good_total || 0) + Number(shift.butal_total || 0));
    const appPartQtyKg = Math.max(0, Number(shift.external_average_weight_grams || 0) / 1000);
    const consumptionRows = parts.map(part => {
      const partKeys = [
        part.part_product_id, part.product_id, part.sku, part.part_sku,
        part.product_sku, part.part_code, part.product_code, part.code,
        part.name, part.part_name, part.material_name,
      ].map(materialKeyText).filter(Boolean);
      const matchedScans = materialRows.filter(scan => {
        const scanKeys = [scan.code, scan.material, scan.material_name, scan.product_id, scan.sku]
          .map(materialKeyText).filter(Boolean);
        return scanKeys.some(key => partKeys.includes(key));
      });
      const scannedQty = matchedScans.reduce((sum, scan) => sum + Math.max(0, Number(scan.qty || 0)), 0);
      const estimate = rawConsumptionForPart(
        matchedScans.length ? matchedScans : materialRows,
        part,
        producedUnits,
      ...
      return {
        ...
        per_unit: firstValue(part.part_qty_per_unit, part.qty_per_unit, appPartQtyKg || "-"),
        produced_units: producedUnits,
        estimated_used: Number(estimate.used || 0),
        scanned_qty: Number(estimate.scanned || scannedQty || 0),
        balance: Number(estimate.scanned || scannedQty || 0) > 0 ? Number(estimate.available || 0) : "-",
      };
    });
    const rows = [
      ["Machine", shift.machine_name || shift.machine_code || "-"],
      ["Work Order No.", job.ref_no || job.code || item.job_code || "-"],
      ["Job Name", item.job_name || job.name || "-"],
      ["Product", job.product_name || details.product_name || "-"],
      ["SKU / System Code", job.product_sku || job.sku || job.custom_09 || "-"],
      ["Mold / Description", details.mold || details.mold_no || job.custom_05 || "-"],
      ["Color", details.color || job.custom_06 || "-"],
      ["Target Quantity", job.approve_qty || job.request_qty || "-"],
      ["Cycle Time", details.std_cycle_time || job.custom_09 || shift.cycle_time_current || "-"],
    ];
    return `
      <div class="pdr-sheet pdr-portrait linked-job-sheet">
        <header class="pdr-topbar">
          <h2>LINKED JOB SUMMARY</h2>
          <div class="pdr-top-meta"><span>${esc(jobDisplayName(item, item.job_code || "Linked Job"))}</span><span>SAME SHIFT</span></div>
        </header>
        <div class="pdr-info">
          ${rows.map(([key,value]) => `<div class="pdr-info-item"><div class="pdr-info-k">${esc(key)}</div><div class="pdr-info-v">${esc(value)}</div></div>`).join("")}
        </div>
        <div class="pdr-kpis">
          <div class="pdr-kpi good"><div class="k">SHIFT GOOD</div><div class="v">${esc(shift.good_total || 0)}</div></div>
          <div class="pdr-kpi butal"><div class="k">SHIFT BUTAL</div><div class="v">${esc(shift.butal_total || 0)}</div></div>
          <div class="pdr-kpi bad"><div class="k">SHIFT REJECT</div><div class="v">${esc(shift.reject_total || 0)}</div></div>
          <div class="pdr-kpi"><div class="k">PACK COUNT</div><div class="v">${esc(shift.pack_count || 0)}</div></div>
          <div class="pdr-kpi"><div class="k">NO SHOT</div><div class="v">${esc(shift.no_shot_total || 0)}</div></div>
          <div class="pdr-kpi"><div class="k">LINK STATUS</div><div class="v">LINKED</div></div>
        </div>
        <section class="pdr-section" style="margin-top:10px;">
          <div class="pdr-section-title">Required Product Parts</div>
          ${tableFromRows(parts, [
            { label: "Product ID", value: x => firstValue(x.part_product_id, x.product_id, x.id, "-") },
            { label: "SKU", value: x => firstValue(x.sku, x.part_sku, x.product_sku, x.code, "-") },
            { label: "Description", value: x => materialLabel(x) },
            { label: "Qty / Unit", value: x => firstValue(x.part_qty_per_unit, x.qty_per_unit, "-") },
          ], "No linked-job product parts in the saved API payload.", 12)}
        </section>
      </div>
    `;
  }

  function linkedJobsForShift(row){
    const item = (row && typeof row === "object") ? row : {};
    const rows = (Array.isArray(item.linkage_jobs) ? item.linkage_jobs : [])
      .filter(linked => linked && typeof linked === "object")
      .map(linked => ({...linked}));
    const primaryCode = String(item.linkage_job_code || "").trim();
    if(primaryCode && !rows.some(linked => String(linked.job_code || "").trim() === primaryCode)){
      rows.push({
        job_code: primaryCode,
        job_name: item.linkage_job_name || primaryCode,
        job_payload: item.linkage_job_payload || {},
      });
    }
    return rows.map(linked => {
      const linkedCode = String(linked.job_code || "").trim();
      const fullRow = (finishedJobsState || []).find(candidate =>
        String(candidate?.job_code || "").trim() === linkedCode && !isShiftPartialRecord(candidate)
      );
      if(fullRow) return fullRow;
      const hasPayload = linked.job_payload && typeof linked.job_payload === "object" && Object.keys(linked.job_payload).length;
      if(!hasPayload && linkedCode === primaryCode){
        linked.job_payload = item.linkage_job_payload || {};
      }
      return linked;
    });
  }
  ...
  function shiftsForSameJob(row){
    ...
    return rows.sort((a, b) =>
      String(a.finished_at_utc || a.ended_at_utc || "").localeCompare(String(b.finished_at_utc || b.ended_at_utc || ""))
    );
  }

  function shiftProductPartsConsumptionHtml(row){
    const item = (row && typeof row === "object") ? row : {};
    const payload = (item.job_payload && typeof item.job_payload === "object") ? item.job_payload : {};
    const data = (payload.data && typeof payload.data === "object") ? payload.data : {};
    const details = (data.job_details && typeof data.job_details === "object") ? data.job_details : {};
    ...
    let targetParts = [];
    if(Array.isArray(data.parts)) targetParts = data.parts;
    else if(Array.isArray(details.parts)) targetParts = details.parts;
    else if(Array.isArray(details.part_ids)) targetParts = details.part_ids;
    else if(details.part_ids && typeof details.part_ids === "object") targetParts = [details.part_ids];
    const combinedParts = [];
    const combinedPartKeys = new Set();
    const addFinishedProductParts = partRows => {
      (Array.isArray(partRows) ? partRows : []).forEach(part => {
        if(!part || typeof part !== "object") return;
        const key = materialKeyText(firstValue(
          part.part_product_id, part.product_id, part.sku, part.part_sku,
          part.product_sku, part.part_code, part.product_code, part.code,
          part.name, part.part_name
        ));
        if(key && combinedPartKeys.has(key)) return;
        if(key) combinedPartKeys.add(key);
        combinedParts.push(part);
      });
    };
    const partsFromPayload = sourcePayload => {
      ...
      const sourceDetails = (sourceData.job_details && typeof sourceData.job_details === "object") ? sourceData.job_details : {};
      if(Array.isArray(sourceData.product_parts) && sourceData.product_parts.length) return sourceData.product_parts;
      if(Array.isArray(sourceData.parts)) return sourceData.parts;
      if(Array.isArray(sourceDetails.parts)) return sourceDetails.parts;
      if(Array.isArray(sourceDetails.part_ids)) return sourceDetails.part_ids;
      return sourceDetails.part_ids && typeof sourceDetails.part_ids === "object" ? [sourceDetails.part_ids] : [];
    };
    addFinishedProductParts(productParts.length ? productParts : targetParts);
    addFinishedProductParts(partsFromPayload(item.linkage_job_payload));
    (Array.isArray(item.linkage_jobs) ? item.linkage_jobs : []).forEach(linked =>
      addFinishedProductParts(partsFromPayload(linked?.job_payload))
    );
    const consumptionCodeKeys = value => [
      value?.part_product_id, value?.product_id,
      value?.sku, value?.part_sku, value?.product_sku,
      value?.part_code, value?.product_code, value?.code,
    ].map(materialKeyText).filter(Boolean);
    const consumptionNameKeys = value => [
      value?.name, value?.material, value?.part_name, value?.material_name,
      value?.product_name, value?.description,
    ].map(materialKeyText).filter(Boolean);
    const matchesConsumptionPart = (part, candidate) => {
      const partCodes = consumptionCodeKeys(part);
      const candidateCodes = consumptionCodeKeys(candidate);
      if(partCodes.length && candidateCodes.length){
        return candidateCodes.some(key => partCodes.includes(key));
      }
      const partNames = consumptionNameKeys(part);
      return consumptionNameKeys(candidate).some(key => partNames.includes(key));
    };
    const hookedProductKeys = new Set();
    const addHookedProductIdentity = (record, recordPayload) => {
      const source = (record && typeof record === "object") ? record : {};
      const sourcePayload = (recordPayload && typeof recordPayload === "object") ? recordPayload : {};
      const sourceData = (sourcePayload.data && typeof sourcePayload.data === "object") ? sourcePayload.data : {};
      const sourceJob = (sourceData.job && typeof sourceData.job === "object") ? sourceData.job : {};
      const sourceDetails = (sourceData.job_details && typeof sourceData.job_details === "object") ? sourceData.job_details : {};
      [
        source.product_id, source.product_sku,
        sourceJob.product_id, sourceJob.product_sku, sourceJob.sku,
        sourceDetails.product_id, sourceDetails.product_sku, sourceDetails.sku,
      ].map(materialKeyText).filter(Boolean).forEach(key => hookedProductKeys.add(key));
    };
    addHookedProductIdentity(item, payload);
    addHookedProductIdentity(
      { product_id: item.linkage_product_id, product_sku: item.linkage_product_sku },
      item.linkage_job_payload
    );
    (Array.isArray(item.linkage_jobs) ? item.linkage_jobs : []).forEach(linked =>
      addHookedProductIdentity(linked, linked?.job_payload)
    );
    const isHookedProducedPart = part =>
      consumptionCodeKeys(part).some(key => hookedProductKeys.has(key));
    const consumptionRows = archivedMaterialRows(item);
    const issuedMaterialMap = new Map();
    consumptionRows.forEach(row => {
      const qty = Number(row?.qty || 0);
      const material = String(row?.material || "").trim();
      const isPrevious = String(row?.source || "").toUpperCase().startsWith("FINISH_SHIFT_PART_CARRYOVER");
      if(isPrevious || !material || material === "-" || !Number.isFinite(qty) || qty <= 0) return;
      const code = String(row?.code || "").trim();
      const key = `${materialKeyText(code)}|${materialKeyText(material)}`;
      const current = issuedMaterialMap.get(key) || {
        material, code, scanned_qty: 0, total_qty: 0,
        lots: new Set(), last_scanned_at: "", scans: [],
      };
      current.scanned_qty += qty;
      ...
      if(lot && lot !== "-") current.lots.add(lot);
      const scanTime = String(row.time || "").trim();
      if(scanTime && (!current.last_scanned_at || scanTime > current.last_scanned_at)){
        current.last_scanned_at = scanTime;
      }
      issuedMaterialMap.set(key, current);
    });
    const issuedMaterialRows = Array.from(issuedMaterialMap.values())
      .sort((a, b) => String(b.last_scanned_at || "").localeCompare(String(a.last_scanned_at || "")))
      .map((row, idx) => ({
        ...row,
        index: idx + 1,
        lots: Array.from(row.lots).join(", ") || "-",
        scans: row.scans
          .sort((a, b) => String(b.time || "").localeCompare(String(a.time || "")))
          .map((scan, scanIndex) => ({ ...scan, index: scanIndex + 1 })),
      }));
    const packRows = (Array.isArray(item.product_pack_history_logs) ? item.product_pack_history_logs : []).filter(x => x && typeof x === "object" && !x.voided);
    const packedQty = packRows.reduce((sum, x) => sum + Math.max(0, Number(x.completed_pack_qty ?? x.qty_q ?? x.good_qty ?? x.qty ?? 0)), 0);
    const actualConsumptionMap = new Map();
    packRows.forEach(pack => {
      (Array.isArray(pack.raw_part_consumption) ? pack.raw_part_consumption : []).forEach(entry => {
        if(!entry || typeof entry !== "object") return;
        const material = firstValue(entry.part_name, entry.material_name, entry.product_name, entry.part_sku, entry.sku, entry.part_code, entry.product_code, "-");
        const code = firstValue(entry.part_sku, entry.sku, entry.part_code, entry.product_code, entry.part_product_id, entry.product_id, "-");
        const unit = String(entry.unit || (entry.part_qty_per_unit_kg != null ? "kg" : "pc")).trim().toLowerCase() || "pc";
        const perUnit = Math.max(0, Number(entry.part_qty_per_unit ?? entry.part_qty_per_unit_kg ?? 0));
        const usedQty = Math.max(0, Number(entry.used_qty ?? entry.used_qty_kg ?? (Number(entry.produced_units || 0) * perUnit)));
        const weightAppliedAt = firstValue(entry.weight_received_at, entry.default_weight_replaced_at, "");
        const baseKey = `${String(code).trim().toLowerCase()}|${String(material).trim().toLowerCase()}`;
        const weightKey = unit === "kg"
          ? `${perUnit.toFixed(9)}|${String(weightAppliedAt || "").trim()}`
          : "component";
        const key = `${baseKey}|${weightKey}`;
        const current = actualConsumptionMap.get(key) || {
          material, code, unit, packed_qty: 0, qty_per_unit: 0, actual_consumed: 0,
          weight_source: new Set(), weight_applied_at: weightAppliedAt,
        };
        current.packed_qty += Math.max(0, Number(entry.produced_units || 0));
        current.qty_per_unit = perUnit || current.qty_per_unit;
        current.actual_consumed += usedQty;
        const source = String(entry.weight_label || entry.part_qty_per_unit_source || "").trim();
        if(source) current.weight_source.add(source);
        actualConsumptionMap.set(key, current);
      });
    });
    for(const [key, consumption] of actualConsumptionMap.entries()){
      const matchesFinishedShiftPart = combinedParts.some(part => matchesConsumptionPart(part, consumption));
      if(!matchesFinishedShiftPart) actualConsumptionMap.delete(key);
    }
    combinedParts.forEach(part => {
      const material = firstValue(part.part_name, part.product_name, part.name, part.material_name, part.description, "-");
      const code = firstValue(part.part_code, part.product_code, part.sku, part.code, "-");
      const key = `${String(code).trim().toLowerCase()}|${String(material).trim().toLowerCase()}`;
      const alreadyRecorded = Array.from(actualConsumptionMap.values()).some(
        consumption => matchesConsumptionPart(part, consumption)
      );
      ...
      const perUnit = Math.max(0, Number(part.part_qty_per_unit || part.qty_per_unit || part.quantity_per_unit || 0));
      actualConsumptionMap.set(key, {
        material, code, unit, packed_qty: packedQty, qty_per_unit: perUnit,
        actual_consumed: packedQty * perUnit, weight_source: new Set(["Job API"]),
        weight_applied_at: "",
      });
    });
    const actualConsumptionRows = Array.from(actualConsumptionMap.values()).map(x => ({ ...x, weight_source: Array.from(x.weight_source).join(", ") || "-" }));
    const partBalanceRows = combinedParts.map(part => {
      const partConsumptionRows = actualConsumptionRows.filter(consumption => matchesConsumptionPart(part, consumption));
      const partIssuedRows = issuedMaterialRows.filter(issued => matchesConsumptionPart(part, issued));
      const consumedQty = partConsumptionRows.reduce(
        (sum, consumption) => sum + Math.max(0, Number(consumption.actual_consumed || 0)), 0
      );
      const scannedIssuedQty = partIssuedRows.reduce(
        (sum, issued) => sum + Math.max(0, Number(issued.total_qty || 0)), 0
      );
      const hookedProduced = isHookedProducedPart(part);
      const issuedQty = hookedProduced ? consumedQty : scannedIssuedQty;
      const difference = issuedQty - consumedQty;
      const unit = partConsumptionRows[0]?.unit || (
        String(firstValue(part.unit, part.uom, part.part_unit, "")).toLowerCase().includes("kg") ? "kg" : "pc"
      );
      return {
        ...part,
        ...
        difference,
        unit,
        hooked_produced: hookedProduced,
        balance_status: hookedProduced
          ? `HOOKED - BALANCE 0.0000 ${unit}`
          : (
              difference < 0
                ? `LACKING ${Math.abs(difference).toFixed(4)} ${unit}`
                : `BALANCE ${difference.toFixed(4)} ${unit}`
            ),
      };
    });
    const trackedProductParts = new Set(
      actualConsumptionRows.map(x => materialKeyText(x.code || x.material)).filter(Boolean)
    ).size;
    const scannedShiftHistoryHtml = issuedMaterialRows.length
      ? `
        <div class="scan-history-list">
          <div class="scan-history-head">
            <span>#</span><span>Material</span><span>Compiled Qty</span><span>Lot</span><span>Last Scanned</span><span></span>
          </div>
          ${issuedMaterialRows.map(item => `
            <details class="scan-history-item">
              <summary class="scan-history-summary">
                ...
                <span class="scan-history-arrow">&#9662;</span>
              </summary>
              <div class="scan-history-body">
                ${tableFromRows(item.scans, [
                  { label:"Scan #", value:scan => scan.index },
                  { label:"Quantity", value:scan => scan.qty },
                  { label:"Lot", value:scan => scan.lot },
                  { label:"Scanned At", value:scan => fmtDateLocal(scan.time || "") || "-" },
                ], "No individual scans recorded.", Math.max(1, item.scans.length))}
              </div>
            </details>
          `).join("")}
        </div>
      `
      : '<div class="machine-detail-empty">No materials scanned during this shift.</div>';
    return `
      <div class="pdr-sheet">
        <header class="pdr-topbar">
          <h2>PRODUCT PARTS &amp; CONSUMPTION</h2>
          <div class="pdr-top-meta"><span>${esc(item.machine_name || item.machine_code || "-")}</span><span>${esc(jobDisplayName(item, "-"))}</span></div>
        </header>
        <div class="consumption-report-layout">
          <aside class="pdr-kpis consumption-kpis">
            <div class="pdr-kpi"><div class="k">ACCEPTED PACKS</div><div class="v">${esc(packRows.length)}</div></div>
            <div class="pdr-kpi good"><div class="k">ACTUAL PACKED QTY</div><div class="v">${esc(packedQty)}</div></div>
          ...
          <div class="review-group-card">
            ...
            <div class="review-group-body">${tableFromRows(actualConsumptionRows, [
              ...
              { label:"Weight Source", value:x => x.weight_source },
              { label:"Weight Applied", value:x => x.unit === "kg" ? (fmtDateLocal(x.weight_applied_at || "") || "-") : "-" },
            ], "No pack-based consumption recorded.", Math.max(1, actualConsumptionRows.length))}</div>
          </div>
          <div class="review-group-card scanned-shift-panel">
            <div class="review-group-head">Scanned This Shift (Latest First)</div>
            <div class="review-group-body">${scannedShiftHistoryHtml}</div>
          </div>
          </div>
        </div>
      </div>
  ...
  function shiftProductConsumptionComparisonHtml(row){
    const shift = (row && typeof row === "object") ? row : {};
    const linkedJobs = linkedJobsForShift(shift);
    if(!linkedJobs.length) return shiftProductPartsConsumptionHtml(shift);
    const mainOnly = {
      ...shift,
    ...
    const linkedReports = linkedJobs.map(linked => shiftProductPartsConsumptionHtml({
      ...shift,
      job_code: linked.job_code || "",
      job_name: linked.job_name || "",
      job_payload: linked.job_payload || {},
      linkage_enabled: false,
      linkage_job_code: "",
      linkage_job_name: "",
      linkage_job_payload: {},
    ...
    return `
      <div class="linked-shift-shell">
        <div class="linked-shift-toolbar">
          <button class="approve-print-btn toggle-linked-job-btn" type="button" aria-expanded="false">Show Linked Job${linkedJobs.length > 1 ? "s" : ""}</button>
        </div>
        <div class="linked-shift-layout linked-hidden">
          ${shiftProductPartsConsumptionHtml(mainOnly)}
          ${linkedReports.join("")}
        </div>
      </div>
    `;
  ...
  function buildShiftPreviewPanels(row){
    ...
    const downtimeHistory = Array.isArray(item.pdr_downtime_logs)
      ? item.pdr_downtime_logs.filter(row => row && typeof row === "object")
      : [];
    const totalDowntimeSeconds = Number(item.downtime_total_seconds || 0) || downtimeHistory.reduce(
      (sum, row) => sum + Math.max(0, Number(row.duration_seconds || 0)), 0
    ) || Math.max(0, Number(item.downtime_last_seconds || 0));
    const downtimeHistoryRows = downtimeHistory.map((row, idx) => {
      const segments = Array.isArray(row.reason_segments) ? row.reason_segments : [];
      return {
        index: idx + 1,
        reasons: segments.length
          ? segments.map(segment => `${segment.reason_code || ""} ${segment.reason || ""}`.trim()).join(" -> ")
          : `${row.reason_code || ""} ${row.reason || "-"}`.trim(),
        duration: fmtDowntimeSeconds(row.duration_seconds || 0),
        maintenance: row.maintenance || "-",
    ...
    const counterOverwriteRows = (Array.isArray(item.machine_counter_overwrite_logs)
      ? item.machine_counter_overwrite_logs
      : []
    ).filter(row => row && typeof row === "object").map((row, idx) => ({
      index: idx + 1,
      old_value: row.old_value ?? "-",
    ...
    const cycleTimeChangeRows = (Array.isArray(item.cycle_time_change_logs)
      ? item.cycle_time_change_logs
      : []
    ).filter(row => row && typeof row === "object").map((row, idx) => ({
      index: idx + 1,
      old_value: row.previous_cycle_time ?? "-",
    ...
    return {
      summary: [
        ...
        { title: "Machine Counter Overwrite Audit", kind: "html", wide: true, content: tableFromRows(counterOverwriteRows, [
          ["#", "index"],
          ["Old Counter", "old_value"],
          ["New Counter", "new_value"],
          ["Supervisor", "supervisor"],
        ...
        { title: "Cycle Time Change History", kind: "html", wide: true, content: tableFromRows(cycleTimeChangeRows, [
          ["#", "index"],
          ["Old", "old_value"],
          ["Current", "new_value"],
          ["Supervisor", "supervisor"],
      ...
      rawCycle: [
        ...
        { title: "Job API Product Parts", kind: "html", content: tableFromRows(productParts, [
          ...
          { label: "Code", value: x => x.part_code || x.product_code || x.code || x.sku || "-" },
          { label: "Qty", value: x => x.qty || x.quantity || x.required_qty || "-" },
        ], "No product parts recorded.", Math.max(1, productParts.length)) },
        { title: "Product Part QR History", kind: "html", wide: true, content: productPartQrHistoryHtml(productParts, item) },
      ...
      downtime: [
        { title: "Downtime", kind: "metrics", wide: true, content: [
          { label: "Total Downtime", value: fmtDowntimeSeconds(totalDowntimeSeconds), tone: totalDowntimeSeconds > 0 ? "warn" : "" },
          { label: "Occurrences", value: downtimeHistory.length },
          { label: "Current / Last Reason", value: `${item.downtime_reason_code || ""} ${item.downtime_reason_text || "-"}`.trim() },
          { label: "Current / Last Duration", value: fmtDowntimeSeconds(item.downtime_last_seconds ?? 0) },
          { label: "Active", value: item.downtime_active ? "YES" : "NO" },
        ] },
        { title: "Downtime History", kind: "html", wide: true, content: tableFromRows(downtimeHistoryRows, [
          ["#", "index"],
          ["Reason(s)", "reasons"],
          ["Duration", "duration"],
          ["Maintenance", "maintenance"],
  ...
  function finishedShiftJobName(row){
    const item = (row && typeof row === "object") ? row : {};
    const job = extractJobRecord(item) || {};
    const payload = item.job_payload && typeof item.job_payload === "object" ? item.job_payload : {};
    return firstValue(
  ...
  function syncFinishedShiftViewButtons(){
    [
      [finishedShiftGridViewBtn, "grid"],
      [finishedShiftListViewBtn, "list"],
    ].forEach(([btn, mode]) => {
      if(!btn) return;
      const active = finishedShiftViewMode === mode;
  ...
  function renderFinishedShiftQueue(rows){
    ...
    syncFinishedShiftViewButtons();
    if(!items.length){
      finishedShiftQueueList.className = "finished-wrap";
      finishedShiftQueueList.innerHTML = '<div class="placeholder">No finished shifts yet.</div>';
      return;
    }
    const sorted = [...items].reverse();
    if(finishedShiftViewMode === "list"){
      finishedShiftQueueList.className = "finished-list-wrap";
      finishedShiftQueueList.innerHTML = `
        <table class="finished-list-table">
          <thead>
            <tr>
              <th>Job Name</th>
              <th>Machine</th>
          ...
          <tbody>
            ${sorted.map((r, idx) => {
              ...
              return `
                <tr>
                  <td class="finished-list-job">${esc(finishedShiftJobName(r))}</td>
                  <td>${esc(machineName)}</td>
                  <td>${esc(fmtDateLocal(r.finished_at_utc || r.ended_at_utc || ""))}</td>
                  <td>${esc(displayNameForId(r.operator_id || "-"))}</td>
                  <td class="finished-list-output">${esc(r.pack_count ?? 0)}</td>
                  <td class="finished-list-output">${esc(r.total_good ?? r.partial_qty ?? 0)}</td>
                  <td class="finished-list-output">${esc(r.reject_total ?? 0)}</td>
                  <td class="finished-list-output">${esc(fmtDowntimeSeconds(r.downtime_last_seconds))}</td>
                  <td><span class="finished-badge">${esc(r.review_status || "PENDING")}</span></td>
                  <td class="finished-list-action"><button class="approve-print-btn shift-review-btn" data-row-index="${idx}" type="button">${isApprovedShiftRecord(r) ? "View Shift" : "Review Shift"}</button></td>
                </tr>
              `;
            }).join("")}
          </tbody>
  ...
  function renderFinishedShiftJobProgress(rows){
    ...
    const groups = Array.from(grouped.entries()).map(([key, list]) => {
      const newestFirst = list.slice().sort((a, b) =>
        String(b.finished_at_utc || b.ended_at_utc || "").localeCompare(String(a.finished_at_utc || a.ended_at_utc || ""))
      );
      ...
      const queueRow = (latestState?.job_queue || []).find(row => String(row?.job_code || "").trim() === String(key));
      const targetQty = Number(queueRow?.target_qty ?? first?.job_payload?.data?.job?.approve_qty ?? first?.job_payload?.data?.job?.request_qty ?? 0);
      const historyProducedQty = Math.max(apiPartialQty, approvedQty);
      const producedQty = Number(queueRow?.produced_now ?? historyProducedQty);
      const remainingQty = Math.max(0, targetQty - producedQty);
      const progressPercent = targetQty > 0 ? Math.max(0, (producedQty / targetQty) * 100) : 0;
      const latestTime = String(first.finished_at_utc || first.ended_at_utc || "");
      const lines = newestFirst.map((row, idx) =>
        `${idx + 1}. ${fmtDateLocal(row.finished_at_utc || row.ended_at_utc || "")} | Shift ${row.shift_index ?? "-"} | Qty ${row.partial_qty || row.total_good || 0} | Operator ${displayNameForId(row.operator_id || "-")} | ${row.review_status || "PENDING"}`
      );
      return { latestTime, html: `
        <div class="finished-item job-progress-card">
          <div class="finished-head">
            <h4>${esc(jobDisplayName(first, key))}</h4>
            <span class="finished-badge">${esc(targetQty > 0 ? `${progressPercent.toFixed(1)}%` : "NO TARGET")}</span>
          </div>
          <div class="finished-grid">
            <div><strong>SKU:</strong> ${esc(jobSku(first) || "-")}</div>
            <div><strong>Total Shifts:</strong> ${esc(list.length)}</div>
            <div><strong>Latest Shift:</strong> ${esc(fmtDateLocal(latestTime))}</div>
            <div><strong>Target Request:</strong> ${esc(targetQty || "-")}</div>
            <div><strong>Current Progress:</strong> ${esc(producedQty)}</div>
            ...
            <div><strong>Progress Source:</strong> ${esc(queueRow ? "Live Job Queue" : "Approved / API History")}</div>
          </div>
          <div class="job-progress-bar" role="progressbar" aria-valuemin="0" aria-valuemax="${esc(targetQty || 0)}" aria-valuenow="${esc(producedQty)}">
    ...
    finishedShiftJobProgress.innerHTML = groups.map(group => group.html).join("");
  }
  ...
  function planningMachineTimingHtml(code, queueRow, cards){
    ...
    return `
      <div class="planning-lane-time">
        ...
        <span>Next start: ${esc(firstPlanned ? fmtDateLocal(nextStartMs) : "-")}</span>
        <span>Next est finish: ${esc(firstEndMs ? fmtDateLocal(firstEndMs) : "-")}${firstQtyPerShift ? ` (${esc(Math.round(firstQtyPerShift))}/shift @ ${esc(firstShiftHours)}h)` : ""}</span>
      </div>
    `;
  }
  ...
  function openApprovePrintOverlay(job){
    ...
    const approvedShiftViewAtOpen = overlayReviewMode === "shift" && isApprovedShiftRecord(activeJobRow);
    const approvePrintOverlayTitle = document.getElementById("approvePrintOverlayTitle");
    if(approvePrintOverlayTitle) approvePrintOverlayTitle.textContent = approvedShiftViewAtOpen
      ? "Production Shift Report"
      : (overlayReviewMode === "shift" ? "Review Shift" : "Approve and Print QR");
    ...
    if(overlayReviewSummaryDisplay) overlayReviewSummaryDisplay.innerHTML = approvedShiftView
      ...
      : shiftPanels ? renderShiftGroupedPanelHtml(shiftPanels.summary, "No shift summary.")
    ...
    if(overlayRawConsumptionDisplay) overlayRawConsumptionDisplay.innerHTML = approvedShiftView
      ...
      : shiftPanels ? renderShiftGroupedPanelHtml(shiftPanels.rawConsumption, "No raw material records.")
  ...
  function closeApprovePrintOverlay(){
    ...
    approvePrintOverlay.classList.remove("linked-shift-report");
    activeJobRow = null;
    overlayReviewSavedApproved = false;
  ...
  function machineCardHtml(s, code, css, statusLabel, flipLinkage = false){
    ...
    const displayedJobCode = String(displayedJob?.job_code || s.job_code || "").trim();
    const mainJobCode = String(s.job_code || "").trim();
    const showingMainJob = !displayedJob || displayedJobCode === mainJobCode;
    const butalByJob = (s.butal_by_job && typeof s.butal_by_job === "object") ? s.butal_by_job : {};
    const jobPackTotal = showingMainJob
      ? Number(s.pack_total || 0)
      : Number(displayedJob?.linkage_pack_count || 0);
    const jobGoodTotal = showingMainJob
      ? Number(s.good_total || 0)
      : Number(displayedJob?.linkage_good_total || 0);
    const jobButalTotal = Number(
      butalByJob[displayedJobCode]
      ?? (showingMainJob ? s.butal_total : 0)
      ?? 0
    );
    // Reject/no-shot records currently belong to the original job unless a
    // per-job allocation is supplied. Do not repeat the main totals while the
    // operator switches to a linked job card.
    const jobRejectTotal = showingMainJob ? Number(s.reject_total || 0) : Number(displayedJob?.reject_total || 0);
    const jobNoShotTotal = showingMainJob ? Number(s.no_shot_total || 0) : Number(displayedJob?.no_shot_total || 0);
    const total = jobGoodTotal + jobButalTotal;
    ...
    const seenLabel = s.last_seen_utc ? fmtDateLocal(s.last_seen_utc) : "-";
    ...
    return `
      ...
      <div class="machine-metrics">
        <div class="machine-metric"><div class="k">Pack</div><div class="v">${esc(jobPackTotal)}</div></div>
        <div class="machine-metric good"><div class="k">Good</div><div class="v">${esc(jobGoodTotal)}</div></div>
        <div class="machine-metric"><div class="k">Butal</div><div class="v">${esc(jobButalTotal)}</div></div>
        <div class="machine-metric bad"><div class="k">Reject</div><div class="v">${esc(jobRejectTotal)}</div></div>
        <div class="machine-metric"><div class="k">No Shot</div><div class="v">${esc(jobNoShotTotal)}</div></div>
  ...
  if(finishShiftViewSelect){
    finishShiftViewSelect.addEventListener("change", () => {
      const host = finishShiftViewSelect.closest(".panel");
      if(!host) return;
      const target = String(finishShiftViewSelect.value || "finishShiftQueuePane");
      host.querySelectorAll(".sub-tab-content").forEach(pane => pane.classList.remove("active"));
      host.querySelector(`#${target}`)?.classList.add("active");
    });
  }
  ...
  settingsMachineClearSelect?.addEventListener("change", () => {
    if(settingsMachineClearBtn) settingsMachineClearBtn.disabled = !settingsMachineClearSelect.value;
    if(settingsMachineClearStatus) settingsMachineClearStatus.textContent = "";
  });
  settingsMachineClearBtn?.addEventListener("click", clearSelectedMachineData);
  settingsClientsRefreshBtn?.addEventListener("click", loadSettingsClientsUi);
  [finishedShiftGridViewBtn, finishedShiftListViewBtn].forEach(btn => {
    btn?.addEventListener("click", () => {
      const nextMode = btn.getAttribute("data-view") === "list" ? "list" : "grid";
      if(finishedShiftViewMode === nextMode) return;
      finishedShiftViewMode = nextMode;
      try {
        localStorage.setItem("finishedShiftViewMode", nextMode);
      } catch(_err) {
        // The selected view still works for this page session.
      }
  ...
  function toggleLinkedJobComparison(ev, root){
    const target = ev.target instanceof Element ? ev.target : null;
    const btn = target?.closest(".toggle-linked-job-btn");
    if(!btn || !root) return;
    const layout = root.querySelector(".linked-shift-layout");
    if(!layout) return;
    const willShow = layout.classList.contains("linked-hidden");
    layout.classList.toggle("linked-hidden", !willShow);
    approvePrintOverlay?.classList.toggle("linked-shift-report", willShow);
    btn.setAttribute("aria-expanded", willShow ? "true" : "false");
    const count = linkedJobsForShift(activeJobRow).length;
    btn.textContent = `${willShow ? "Hide" : "Show"} Linked Job${count > 1 ? "s" : ""}`;
  }
  overlayReviewSummaryDisplay?.addEventListener("click", ev => toggleLinkedJobComparison(ev, overlayReviewSummaryDisplay));
  overlayRawConsumptionDisplay?.addEventListener("click", ev => toggleLinkedJobComparison(ev, overlayRawConsumptionDisplay));
  overlayReviewSummaryDisplay?.addEventListener("change", (ev) => {
    const target = ev.target instanceof Element ? ev.target : null;
    const select = target?.closest(".same-job-shift-select");
    if(!select || !activeJobRow) return;
    const idx = Number(select.value);
    const shift = shiftsForSameJob(activeJobRow)[idx];
    if(!shift || jobKeyOf(shift) === jobKeyOf(activeJobRow)) return;
    openApprovePrintOverlay(shift);
    setOverlayStep("review");
  });
  ...
  const qrViewOverlay = document.getElementById("qrViewOverlay");
  const closeQrViewer = () => {
    qrViewOverlay?.classList.remove("active");
    const image = document.getElementById("qrViewImage");
    if(image) image.innerHTML = "";
  };
  document.getElementById("qrViewCloseBtn")?.addEventListener("click", closeQrViewer);
  qrViewOverlay?.addEventListener("click", ev => {
    if(ev.target === qrViewOverlay) closeQrViewer();
  });
  document.addEventListener("click", ev => {
    const button = ev.target.closest(".qr-view-btn");
    if(!button) return;
    ev.preventDefault();
    ev.stopPropagation();
    try {
      openQrViewer(JSON.parse(decodeURIComponent(button.dataset.qrView || "")));
    } catch(err) {
      alert(`Unable to open QR details: ${err?.message || err}`);
    }
  });
  document.addEventListener("keydown", ev => {
    if(ev.key === "Escape" && qrViewOverlay?.classList.contains("active")) closeQrViewer();
  });
...
<script>
  ...
  async function loadProfiles(){
    try{
      ...
      const out = await r.json();
      if(!r.ok || !out.ok) throw new Error(out.error || 'Profile request failed.');
      const rows = Array.isArray(out.items) ? out.items : [];
      pfTableBody.innerHTML = rows.slice().reverse().map(x => `<tr>
        ...
        <td>${esc(x.role)}</td>
        <td>${esc(fmtDateLocal(x.created_at_utc))}</td>
        <td>${esc(x.last_printed_at_utc ? fmtDateLocal(x.last_printed_at_utc) : "-")}</td>
      ...
      setStatus(`${rows.length} saved profile${rows.length === 1 ? '' : 's'} loaded.`);
    }catch(err){
      pfTableBody.innerHTML = `<tr><td colspan="8">Failed to load profiles: ${esc(err?.message || err)}</td></tr>`;
...
def api_profiles():
    global PROFILES
    try:
        latest_profiles = load_profiles()
        PROFILES[:] = [row for row in latest_profiles if isinstance(row, dict)]
    except Exception as exc:
        _append_server_app_log("profiles_sql_refresh_failed", {"error": str(exc)})
...
def _session_snapshot_is_stale(sess: MachineSession, snap: Dict[str, Any]) -> bool:
    ...
    Replayed machine/status events can move the server's segment timestamp
    ...
    same_job = bool(snap_job_key and current_job_key and snap_job_key == current_job_key)
    incoming_pack_logs = [
        row for row in (snap.get("product_pack_history_logs") or [])
        if isinstance(row, dict)
        and str(row.get("source") or "").strip().upper() != "PACK_EVENT_FALLBACK"
    ] if isinstance(snap.get("product_pack_history_logs"), list) else []
    history_advances = len(incoming_pack_logs) > len(sess.product_pack_history_logs or [])
    try:
        snap_pack = int(snap.get("pack_total", snap.get("pack_count", -1)))
        snap_good = int(snap.get("good_total", -1))
    ...
    return not (
        ...
        and (
            history_advances
            or (
                snap_pack >= int(sess.pack_total or 0)
                and snap_good >= int(sess.good_total or 0)
            )
        )
    )
...
def _apply_embedded_session_snapshot(sess: MachineSession, snap: Any, machine_code: str) -> bool:
    ...
    if isinstance(snap.get("linkage_jobs"), list):
        sess.linkage_jobs = [dict(row) for row in snap.get("linkage_jobs") or [] if isinstance(row, dict)]
    sess.linkage_enabled = bool(snap.get("linkage_enabled", bool(sess.linkage_jobs)))
    sess.linkage_job_code = snap.get("linkage_job_code", sess.linkage_job_code)
    sess.linkage_job_name = snap.get("linkage_job_name", sess.linkage_job_name)
    if isinstance(snap.get("linkage_job_payload"), dict):
        sess.linkage_job_payload = dict(snap.get("linkage_job_payload") or {})
    ...
    for key in (
        ...
        if key in snap:
            setattr(sess, key, bool(snap.get(key)))
    for key in (
        "downtime_reason_code",
        "downtime_reason_text",
        "downtime_started_at",
        "downtime_last_seconds",
        "pdr_operator_reason_code",
        "pdr_operator_reason_text",
        "downtime_wait_started_at",
        "downtime_wait_last_seconds",
        "supervisor_downtime_confirmation_started_at",
        "maintenance_name",
        "supervisor_name",
    ):
        if key in snap:
            setattr(sess, key, snap.get(key))
    if isinstance(snap.get("pdr_downtime_logs"), list):
        sess.pdr_downtime_logs = [
...
async def api_event(req: Request):
    ...
    preliminary_event = data.get("event") if isinstance(data.get("event"), dict) else {}
    preliminary_event_type = str(preliminary_event.get("type") or "").strip().upper()
    if event_id and event_id in PROCESSED_EVENT_ID_SET:
        if preliminary_event_type not in {"HEARTBEAT", "PING", "CLIENT_STATUS"}:
            _append_server_app_log(
    ...
    if identity_owner is not None:
        conflict_payload = {
            ...
            "client_id": client_id,
            "owner_remote_host": str(identity_owner.get("remote_host") or ""),
            "owner_last_seen_utc": str(identity_owner.get("last_seen_utc") or ""),
        }
        identity_owner["identity_conflict"] = True
        identity_owner["conflict_remote_host"] = remote_host
        identity_owner["conflict_last_seen_utc"] = utc_now().isoformat()
        _append_server_app_log("client_identity_conflict", conflict_payload)
        await broadcast_state()
        return JSONResponse(
            {
                "ok": False,
                "error_code": "CLIENT_IDENTITY_CONFLICT",
                "error": (
                    f"Client ID {client_id} is already active from "
                    f"{conflict_payload['owner_remote_host']}"
                ),
                **conflict_payload,
            },
            status_code=409,
    ...
    event_created_at = _parse_iso_utc(data.get("event_created_at_utc"))
    ...
    if stale_queued_event:
        ...
        # must not update live state, fill the processed-ID store, or flood the
        # application log while an offline client's queue is being drained.
        return {
            "ok": True,
            "ignored": True,
            "discarded": True,
            "reason": "stale queued event; waiting for current live session snapshot",
        }

    if ev_type in ("FINISH_JOB", "FINISH_SHIFT") and not SERVER_ACCEPT_FINISH_EVENTS:
        finished_payload = ev.get("finished_job") if isinstance(ev.get("finished_job"), dict) else {}
        finished_at = _parse_iso_utc(finished_payload.get("finished_at_utc"))
        if finished_at is None or finished_at < SERVER_FINISH_EVENT_CUTOFF_UTC:
            # Acknowledge stale client history so its retry queue drains without
            # rebuilding the cleared server history. Finishes created after this
            # server start continue through the normal storage/broadcast path.
            return {
                "ok": True,
                "ignored": True,
                "reason": "finish event predates the current server session",
            }
    ...
    stale_live_context_cleared = False
    if allow_common_identity_update:
        incoming_job_keys = {
            str(value or "").strip().upper()
            for value in (incoming_job_code, incoming_job_name)
            if str(value or "").strip()
        }
        payload_job_keys = _job_payload_identity_keys(sess.job_payload)
        if (
            ev_type == "HEARTBEAT"
            and incoming_job_keys
            and payload_job_keys
            and incoming_job_keys.isdisjoint(payload_job_keys)
        ):
            _append_server_app_log(
                "stale_live_job_context_cleared",
                {
                    "client_id": client_id,
                    "machine_code": machine_code,
                    "incoming_job": sorted(incoming_job_keys),
                    "payload_job": sorted(payload_job_keys),
                    "cleared_waiting_downtime_start_maintenance": bool(
                        sess.waiting_downtime_start_maintenance
                    ),
                },
            )
            _clear_stale_job_payload_and_pdr_wait(sess)
            stale_live_context_cleared = True
    ...
    if ev_type == "FINISH_SHIFT":
        ...
        downtime_record = ev.get("downtime_record")
        if isinstance(downtime_record, dict):
            rows = list(sess.pdr_downtime_logs or [])
            record_index = int(downtime_record.get("downtime_index") or 0)
            rows = [
                row for row in rows
                if not (
                    isinstance(row, dict)
                    and record_index > 0
                    and int(row.get("downtime_index") or 0) == record_index
                )
            ]
            rows.append(dict(downtime_record))
            sess.pdr_downtime_logs = rows
        _clear_resolved_pdr_workflow(sess)
    elif ev_type == "PRODUCTION_DAILY_REPORT":
        mode = str(ev.get("mode") or "").strip().upper()
        if mode == "DOWNTIME_STARTED":
            sess.pdr_operator_reason_code = None
            sess.pdr_operator_reason_text = None
            sess.downtime_reason_code = None
            sess.downtime_reason_text = None
            sess.waiting_downtime_start_maintenance = False
            sess.waiting_pdr_maintenance_reason = False
            sess.waiting_downtime_end_maintenance = True
            sess.downtime_started_at = time.time()
            sess.downtime_last_seconds = 0
            sess.downtime_active = True
            sess.maintenance_name = None
        elif mode == "WAITING_FOR_MAINTENANCE":
            sess.pdr_operator_reason_code = None
            sess.pdr_operator_reason_text = None
            sess.downtime_reason_code = None
            sess.downtime_reason_text = None
    ...
    elif ev_type == "MACHINE_COUNTER_OVERWRITE":
        counter_change = ev.get("counter_change")
        if isinstance(counter_change, dict):
            rows = list(sess.machine_counter_overwrite_logs or [])
            change_time = str(counter_change.get("changed_at_utc") or "").strip()
            rows = [
                row for row in rows
                if not (
                    isinstance(row, dict)
                    and change_time
                    and str(row.get("changed_at_utc") or "").strip() == change_time
                )
            ]
            rows.append(dict(counter_change))
            sess.machine_counter_overwrite_logs = rows
            sess.machine_counter_current = _parse_int_or_none(counter_change.get("new_value"))
            _append_server_app_log(
                "machine_counter_overwrite",
                {
                    "client_id": client_id,
                    "machine_code": machine_code,
                    "old_value": counter_change.get("old_value"),
                    "new_value": counter_change.get("new_value"),
                    "supervisor_code": counter_change.get("supervisor_code"),
                    "supervisor_name": counter_change.get("supervisor_name"),
                    "job_code": counter_change.get("job_code"),
                    "operator_id": counter_change.get("operator_id"),
                    "changed_at_utc": counter_change.get("changed_at_utc"),
                },
            )
    ...
    elif ev_type == "JOB_LINKAGE_REMOVED":
        removed_code = str(ev.get("removed_job_code") or "").strip()
        removed_name = str(ev.get("removed_job_name") or "").strip()
        sess.linkage_jobs = [
            row for row in (sess.linkage_jobs or [])
            if isinstance(row, dict)
            and not (
                removed_code
                and str(row.get("job_code") or "").strip().casefold() == removed_code.casefold()
            )
            and not (
                not removed_code
                and removed_name
                and str(row.get("job_name") or "").strip().casefold() == removed_name.casefold()
            )
        ]
        sess.linkage_enabled = bool(sess.linkage_jobs)
        if str(sess.linkage_job_code or "").strip().casefold() == removed_code.casefold():
            primary = sess.linkage_jobs[-1] if sess.linkage_jobs else {}
            sess.linkage_job_code = primary.get("job_code")
            sess.linkage_job_name = primary.get("job_name")
            sess.linkage_job_payload = primary.get("job_payload") or {}
        _append_server_app_log(
            "job_linkage_removed",
            {
                "client_id": client_id,
                "machine_code": machine_code,
                "removed_job_code": removed_code,
                "removed_job_name": removed_name,
                "remaining_linked_jobs": len(sess.linkage_jobs),
            },
        )
        ...
        if isinstance(snap, dict):
            ...
            if isinstance(snap.get("cycle_time_change_logs"), list):
                sess.cycle_time_change_logs = [
                    dict(row) for row in snap.get("cycle_time_change_logs") or []
                    if isinstance(row, dict)
                ]
...
def api_active_sessions(client_id: str = "", machine_code: str = ""):
    ...
    # The Raspberry Pi client polls this endpoint while it is running.  Treat a
    # client-specific poll as a presence signal so a retry backlog cannot leave
    # the machine marked disconnected while fresh heartbeats wait in the event
    # queue.  Unfiltered dashboard/admin reads must not affect presence.
    if cid and code_filter:
        polled_session = SESSIONS.get(code_filter)
        if polled_session is None:
            # A stale production session may have been intentionally discarded.
            # Recreate only a blank online shell from the live client's poll;
            # never restore the discarded job, operator, counters, or logs.
            polled_session = MachineSession(
                client_id=cid,
                machine_code=code_filter,
                machine_name=_machine_display_name(code_filter, code_filter),
                last_event="CLIENT ONLINE - WAITING FOR FRESH SESSION",
            )
            SESSIONS[code_filter] = polled_session
        if polled_session is not None and str(polled_session.client_id or "").strip() == cid:
            polled_session.last_seen_utc = utc_now().isoformat()
            _update_client_status(
                cid,
                "",
                machine_code=code_filter,
                machine_name=polled_session.machine_name,
                event_type="CLIENT_POLL",
                last_event=str(polled_session.last_event or "CLIENT ONLINE"),
                machine_scanned=True,
            )
    rows = []
    for sess in SESSIONS.values():
...
@APP.get("/api/client-identities")
def api_client_identities():
    """Return every reserved client ID, including clients currently offline."""
    identities: set[str] = set()

    def add(value: Any) -> None:
        text = str(value or "").strip()
        if text and text.upper() not in {"UNKNOWN", "SNAPSHOT"}:
            identities.add(text)

    for row in _client_status_rows():
        add(row.get("client_id"))
    for session in SESSIONS.values():
        add(session.client_id)
    for row in FINISHED_JOBS:
        if isinstance(row, dict):
            add(row.get("client_id"))
    for row in ARCHIVED_JOBS:
        if isinstance(row, dict):
            add(row.get("client_id"))

    highest = 0
    for identity in identities:
        match = re.fullmatch(r"RPI-CLIENT-(\d+)", identity, flags=re.IGNORECASE)
        if match:
            highest = max(highest, int(match.group(1)))
    suggested = f"RPI-CLIENT-{highest + 1:02d}"
    return {
        "ok": True,
        "items": sorted(identities, key=lambda value: value.upper()),
...
async def api_daily_roles_save(req: Request):
    ...
    if extra_privilege not in ("none", "supervisor", "qc", "maintenance", "operator"):
        return JSONResponse(
            {"ok": False, "error": "extra_privilege must be none, supervisor, qc, maintenance, or operator"},
            status_code=400,
        )
...
@APP.post("/api/qr-view")
async def api_qr_view(req: Request):
    data = await req.json()
    payload = str(data.get("payload", ""))
    if not payload:
        return JSONResponse({"ok": False, "error": "QR payload is empty"}, status_code=400)
    if len(payload) > 4096:
        return JSONResponse({"ok": False, "error": "QR payload is too long"}, status_code=400)
    try:
        image_data_url = _qr_png_data_url(payload)
    except Exception as exc:
        return JSONResponse({"ok": False, "error": f"Unable to create QR: {exc}"}, status_code=400)
...
if __name__ == "__main__":
    import uvicorn
    # Python 3.11's Windows Proactor loop can assert in _attach() when an
    # incoming connection races with listener shutdown.  The selector loop is
    # sufficient for uvicorn and shuts down cleanly on Windows.
    if os.name == "nt":
        asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())
    host = os.environ.get("MACHINE_SERVER_HOST", "0.0.0.0").strip() or "0.0.0.0"
